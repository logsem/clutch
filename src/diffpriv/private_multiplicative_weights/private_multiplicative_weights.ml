...
let mw_l db q v eta =
  if c_query_l q db < v
  then normalize_l (List.mapi (fun i x -> exp (-. eta *. (if List.nth q i then 0. else 1.)) *. x) db)
  else normalize_l (List.mapi (fun i x -> exp (-. eta *. (if List.nth q i then 1. else 0.)) *. x) db)
...
let oPMW_l ?(gif=None) size db index unif stream_q card_q num den alpha beta =
  ...
  let rec aux i bs distrib =
    ...
    | Some q -> (
        if i >= int_of_float c then (* we retrun only from the distribution *)
          aux i ((abs_f (c_query_l q db -. c_query_l q distrib)) :: bs) distrib
        else (
          (match f (fun x' -> int_of_float ((float_of_int size) *. (c_query_l q x' -. c_query_l q distrib))) with
          | None -> ( match f (fun x' -> int_of_float ((float_of_int size) *. (c_query_l q distrib -. c_query_l q x'))) with
              | None -> aux i ((abs_f (1. -. c_query_l q db /. c_query_l q distrib)) :: bs) distrib
              | Some v -> (
                  write_db_l index distrib gif (i +1) ;
                  let a = c_query_l q distrib -. float_of_int v /. (float_of_int size) in
                  aux (i + 1) ((abs_f (1. -. c_query_l q db /. a)) :: bs) (mw_l distrib q a  (alpha /. 2.))))
          | Some v -> (
              write_db_l index distrib gif (i +1) ;
              let a = c_query_l q distrib +. float_of_int v /. (float_of_int size) in
              aux (i + 1) ((abs_f (1.-. c_query_l q db /. a)) :: bs) (mw_l distrib q a  (alpha /. 2.)))
