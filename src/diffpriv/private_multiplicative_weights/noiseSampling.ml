...
module Q = struct
  ...
  let mult (x_num, x_den) (y_num, y_den) =
    normalize (x_num * y_num, x_den * y_den)
end

let sample_distr d n =
  let rec f n xs = if n = 0 then xs else f (n - 1) (d () :: xs) in
  f n []

let geo () =
  let rec geo_rec n = if Random.bool () then n else geo_rec (n + 1) in
  geo_rec 0

(* Most of the sampling code below is hand-ported from SampCert *)
let rec probWhile cond body state =
  if cond state then probWhile cond body (body state) else state

let probUntil body cond =
  probWhile (fun v -> not (cond v)) (fun _st -> body ()) (body ())

let bernoulli_sample num den =
  let d = Random.full_int den in
  d < num

let bernoulliExpNegSampleUnitLoop num den (_, st2) : bool * int =
  let a = bernoulli_sample num (st2 * den) in
  (a, st2 + 1)
...
let bernoulliExpNegSampleUnit num den =
  let k = bernoulliExpNegSampleUnitAux num den in
  k mod 2 = 0
...
let rec bernoulliExpNegSampleGenLoop iter =
  ...
  | 0 -> true
  | _ ->
      let b = bernoulliExpNegSampleUnit 1 1 in
    ...
    let b =
      bernoulliExpNegSampleGenLoop
        (let gamf = num / den in
         gamf)
    in
    if b then bernoulliExpNegSampleUnit (num mod den) den else false
...
let discreteLaplaceSampleLoopIn2Aux num den k =
  let a = bernoulliExpNegSample num den in
  (a, snd k + 1)
...
let discreteLaplaceSample num den : int =
  let r =
    probUntil
      (fun () -> discreteLaplaceSampleLoop num den)
      (fun x -> not (fst x && snd x = 0))
  in
