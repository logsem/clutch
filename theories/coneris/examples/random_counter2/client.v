  ...
  Proof.
    ...
    - wp_apply (incr_counter_spec _ _ _ _ (λ _ _, ∃ n : nat, own γ1 (◯E (Some n)) ∗ counter_content_frag γ (1 / 2) n )%I with "[$Hcounter Hfrag1 Hc1]"); [done| |by iIntros].
      ...
      + iDestruct "Herr" as "(%&Herr&->)/=".
        iMod (counter_tapes_presample _ _ _ _ _ _ (λ x, if 0<? fin_to_nat x then 0%R else (Rpower 4 (1+(bool_to_nat (bool_decide (n2 = Some 0%nat)) - 2))))%R with "[$][$][$]") as "(%n & Herr & Htape)".
        * apply namespaces.coPset_subseteq_difference_r; last done.
          by apply ndot_ne_disjoint.
      ...
      + iMod (counter_tapes_presample _ _ _ _ _ _  (λ _, 0%R) with "[$][$][$]") as "(%n & Herr & Htape)".
        ...
        * iMod (ghost_var_update _ (Some (fin_to_nat n)) with "[$Hauth2][$]") as "[Hauth2 Hfrag2]".
          ...
          -- iIntros (z) "Hauth1".
             ...
             by iFrame.
      + iDestruct "Herr" as "(%&Herr&->)/=".
        iMod (counter_tapes_presample _ _ _ _ _ _ (λ x, if 0<? fin_to_nat x then 0%R else (Rpower 4 (1+((bool_to_nat (bool_decide (n1 = Some 0)) + 0)%nat - 2))))%R with "[$][$][$]") as "(%n & Herr & Htape)".
        * apply namespaces.coPset_subseteq_difference_r; last done.
          by apply ndot_ne_disjoint.
        * intros. case_match; first done. rewrite /Rpower. left. apply exp_pos.
        * rewrite SeriesC_finite_foldr/=.
