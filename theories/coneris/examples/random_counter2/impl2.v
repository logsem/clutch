...
From coneris.coneris Require Import coneris random_counter2.random_counter hocap_flip.
...
Lemma incr_counter_spec2 N E c γ1 (Q:nat->nat->iProp Σ)  :
    ↑N⊆E ->
    {{{ is_counter2 N c γ1 ∗
        (∀ α, (flip_tapes (L:=L) α (expander []) ∗ ⌜Forall (λ x, x<4) []⌝) -∗
         state_update (E) (E)
           (∃ n, (flip_tapes (L:=L) α (expander ([n])) ∗ ⌜Forall (λ x, x<4) ([n])⌝) ∗
  ...
  Proof.
    ...
    wp_pures.
    iMod ("Hvs" with "[$Htape //]") as "(%n & (Htape&%H) & Hvs)".
