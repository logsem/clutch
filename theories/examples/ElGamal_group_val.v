...
From clutch Require ElGamal_bijection.
...
Variable cg : clutch_group_struct.
Variable G : clutch_group (vg:=vg) (cg:=cg).
Coercion vg_of_cg := λ (vg : val_group) (cg : clutch_group_struct) (G : clutch_group (vg := vg) (cg:=cg)), vg.
...
Let is_id_vid := is_unit.
Let vinv : val := vinv.
Let is_inv_vinv := is_inv.
Let eexp : val := vexp.
Let is_exp_eexp := is_exp.
Let is_spec_exp_eexp := is_spec_exp.
...
Hypothesis g_gen : generator [set: vg] g.

Fact mult_typed : ∀ Γ, Γ ⊢ₜ vmult : (τ → τ → τ)%ty.
Proof. intros. constructor. apply vmult_typed. Qed.
Fact eexp_typed : ⊢ refines top eexp eexp (T → lrel_int → T).
Proof.
...
Fact log_g : ∀ v : vg, ∃ k : fin (S n), (g^+k = v)%g.
Proof.
...
Proof with rel_pures_l ; rel_pures_r.
  ...
  all: rel_apply refines_app ;
    [ rel_apply refines_app ; [ iApply eexp_typed |] ;
...
Proof with rel_pures.
  ...
  - rel_load_r ; rel_load_l...
    ...
    rewrite -H.
    pose (f := ElGamal_bijection.f n'' k).
    unshelve rel_apply (refines_couple_tapes n f with "[- $γ $γ']") => //.
    1: constructor ; [apply ElGamal_bijection.f_inj|apply ElGamal_bijection.f_surj].
    iIntros (c) "[γ' γ]"...
    rel_rand_l ; rel_rand_r.
...
(* Variable vg : val_group. *)
(* Variable cg : clutch_group_struct. *)
(* Goal `{clutchGpreS} Type. *)
(*   unshelve eapply (@clutch_group _ _ vg). *)
(*   1: exact clutchRΣ. *)

(* Variable G : clutch_group (vg:=vg). *)

(* Local Notation "'τ'" := (τG G). *)
(* Definition T := interp.interp τ []. *)
...
Lemma ctx_pk_ots_rnd_real_real_lbl :
  forall vg cg,
  forall (G : forall `{!clutchRGS Σ}, clutch_group (vg:=vg) (cg:=cg)),
  forall (cgg : clutch_group_generator),
  (* forall `{HΣ : ! clutchRGpreS Σ}, *)
    let e := @pk_ots_rnd_real vg cg g n'' in
    let e' := pk_ots_rnd_real_lbl vg cg g n'' in
    let τ := ((() → τG) * (τG → τG * τG))%ty in
    ∅ ⊨ e ≤ctx≤ e' : τ.
Proof.
  intros.
  apply (refines_sound clutchRΣ).
  intros => /=.
  rewrite (@τG_closed _ _ _ _ (G clutchRΣ _) _).
  apply (@pk_ots_rnd_real_real_lbl _ _ vg cg (G clutchRΣ _) g n'' g_nontriv g_generator).
Qed.
...
Lemma ctx_pk_ots_rnd_real_lbl_0 :
  ...
  (* forall `{HΣ : ! clutchRGpreS Σ}, *)
    let e := @pk_ots_rnd_real_lbl vg cg g n'' in
    let e' := pk_ots_rnd_0 vg cg g n'' in
    let τ := ((() → τG) * (τG → τG * τG))%ty in
    ∅ ⊨ e ≤ctx≤ e' : τ.
Proof.
  intros.
  ...
  intros => /=.
  rewrite (@τG_closed _ _ _ _ (G clutchRΣ _) _).
  apply (@pk_ots_rnd_real_lbl_0 _ _ vg cg (G clutchRΣ _) g n'' g_nontriv g_generator).
Qed.

Lemma ctx_pk_ots_rnd_real_0 :
  forall vg cg,
  forall (G : forall `{!clutchRGS Σ}, clutch_group (vg:=vg) (cg:=cg)),
  forall (cgg : clutch_group_generator),
  (* forall `{HΣ : ! clutchRGpreS Σ}, *)
...
Proof.
  intros.
  eapply (ctx_refines_transitive _ _ _ (pk_ots_rnd_real_lbl _ _ _ n'')).
  - apply ctx_pk_ots_rnd_real_real_lbl. apply G.
  - apply ctx_pk_ots_rnd_real_lbl_0. apply G.
Qed.
...
Lemma pk_ots_rnd_real_real_ddh (* vmult vexp vunit *) :
  ...
  forall vg cg,
  forall `{HΣ : ! clutchRGS Σ},
  forall (G : clutch_group (vg:=vg) (cg:=cg)),
  forall (cgg : clutch_group_gen),
  (* forall `{HΣ : ! clutchRGpreS Σ}, *)
  ∅ ⊨ pk_ots_rnd_real _ _ G g n'' ≤ctx≤ fill (C _) (DH_real_lbl _ _ G g n'') : ((() → τG) * (τG → τG * τG)).
Proof.
  intros.
  rewrite -pk_ots_rnd_1_dh_real_lbl => //.
  2,3: destruct cgg ; auto.
  set (e := pk_ots_rnd_real vg cg G g n'').
  set (e' := pk_ots_rnd_1 vg cg G g n'').
  set (τ := ((() → τG) * (τG → τG * τG))%ty).

  (* Unset Printing Notations. *)
  (* Set Printing Implicit. *)
  (* Set Printing Coercions. *)
  assert (forall H, H = HΣ) as HΣ1 by admit.
  assert (closed : forall Σ H Δ, @interp Σ H τG Δ = @interp Σ H τG []) by admit.
  eapply (ctx_refines_transitive _ _ _ (pk_ots_rnd_real_lbl _ _ _ g n'')).
  - unshelve eapply (@refines_sound Σ _).
    { destruct HΣ. constructor => //.
      destruct clutchRGS_clutchGS.
    ...
    (*       ). *)
    clear.
    epose (@pk_ots_rnd_real_real_lbl).

    (λ Σ HΣ,
      @pk_ots_rnd_real_real_lbl vg _ _ g n'' g_nontriv g_gen)

    intros. simpl. rewrite closed.
    pose (pk_ots_rnd_real_real_lbl vg _ _ g n'' g_nontriv g_gen) as h. (* unfold T in h => /=. *)
    rewrite (HΣ1 H).
    exact h.
  - eapply (ctx_refines_transitive _ _ _ (pk_ots_rnd_0 _ _ _ g n'')).
    + eapply (@refines_sound Σ _) ; intros.
      pose (pk_ots_rnd_real_lbl_0 vg _ _ g n'' g_nontriv g_gen) as h.
      unfold T in h => /=.
      rewrite (HΣ1 H).
      rewrite closed.
      exact h.
    + eapply (@refines_sound Σ _) ; intros.
      pose (pk_ots_rnd_0_1 vg _ _ g n'' g_nontriv g_gen) as h.
      unfold T in h => /=.
      rewrite (HΣ1 H).
      rewrite closed.
      exact h.
