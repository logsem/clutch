...
Section keyed_hash.
  ...
  Proof.
    rewrite /key_of_enc_gallina /enc_gallina/MAX_VALS.
  ...
  Proof.
    rewrite /key_of_enc_gallina /enc_gallina/MAX_VALS/MAX_KEYS/MAX_HASH_DOM.
    rewrite ?Nat.shiftr_div_pow2 ?Nat.shiftl_mul_pow2.
    specialize (pow2_nonzero MAX_VALS_POW) => ?.
    intros Hle.
    transitivity (MAX_HASH_DOM `div` 2 ^ MAX_VALS_POW); last first.
    { cut (MAX_HASH_DOM `div` 2 ^ MAX_VALS_POW = MAX_KEYS).
      { intros ->. rewrite /MAX_KEYS. lia. }
      symmetry. Search Nat.div "mod".
      rewrite /MAX_HASH_DOM.
  Abort.
  (*


    cut (x `div` 2 ^ MAX_VALS_POW + 1 ≤ 2 ^ MAX_KEYS_POW); try lia; [].
    rewrite plus_comm -Nat.div_add_l; last lia.
    transitivity (1 * 2 ^ MAX_VALS_POW + (2 ^ (MAX_KEYS_POW + MAX_VALS_POW) - 1) 

    { assert (x `div` 2 ^ MAX_VALS_POW + 2 ^ MAX_VALS_POW `div` 2 ^ MAX_VALS_POW
                = ((1 * 2^MAX_VALS_POW + x) `div` 2^MAX_VALS_POW)).
      { rewrite plus_comm. rewrite Nat.div_add_l; last lia.
    ...
    etransitivity.
    { eapply (Nat.div_le_mono _ MAX_HASH_DOM); last eapply Hle; lia. }
    rewrite /MAX_HASH_DOM.
    transitivity (Z.to_nat ((Z.pow 2 (MAX_KEYS_POW + MAX_VALS_POW) - 1) / (2 ^ MAX_VALS_POW))%Z).
    { admit. }
    rewrite /Z.sub.
    rewrite Zplus_comm.
