...
(* Katz Lindell, Def. 12.2.: the eavesdropping indistinguishability experiment
   for a public key scheme *)
Definition PubK_eav (keygen enc : val) : expr :=
  λ: "A",
    let: "pk_sk" := keygen #() in
    let: "pk" := Fst "pk_sk" in
    let: "sk" := Snd "pk_sk" in
    let: "m1m2" := "A" (int_of_vg "pk") in
    let: "b" := rand #1 from #() in
    let:m "vm1" := vg_of_int "m1" in
    let:m "vm2" := vg_of_int "m2" in
    let: "m" := if: "b" = #0 then Fst "m1m2" else Snd "m1m2" in
    let: "c" := enc "pk" "m" in
    let: "b'" := "A" (int_of_vg "c") in
    SOME ("b" = "b'").
...
Lemma ElGamal_correct :
  ⊢ refines top
      ...
      (λ:"msg",
         ...
         SOME (int_of_vg "m")) (lrel_int → () + lrel_int).
Proof with rel_pures.
  ...
  1: rel_vals. rel_bind_l (rnd _). rel_apply_l refines_wp_l. iApply wp_rand => //.
