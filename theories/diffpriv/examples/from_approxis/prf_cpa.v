...
From clutch.diffpriv.examples Require Import prf symmetric security_aux option xor.
...
*)
    ...
    Proof with (rel_pures_l ; rel_pures_r).
      ...
      rel_apply (refines_couple_UU Key (λ x, x)) => //.
      iIntros (key) "!> %"...
  ...
  Lemma rf_CPA_ARC `{diffprivRGpreS Σ} `{forall foo, @XOR_spec Σ foo _ _ xor_struct} σ σ' (Q : nat) :
    ARcoupl
      ...
      ((Q-1) * Q / (2 * S Input)).
  Proof.
    unshelve eapply DPcoupl_to_ARcoupl, approximates_coupling ; eauto.
  ...
  Lemma rf_CPA_ARC' Σ `{diffprivRGpreS Σ} `{forall foo, @XOR_spec Σ foo _ _ xor_struct} σ σ' (Q : nat) :
    ARcoupl
      (lim_exec ((CPA #false adv rf_scheme #Q), σ))
      (lim_exec ((CPA #true adv rf_scheme #Q), σ'))
      (=)
  ...
  Corollary CPA_bound_1 Σ `{diffprivRGpreS Σ} `{forall foo, @XOR_spec Σ foo _ _ xor_struct} σ σ' (Q : nat) :
  ...
  Corollary CPA_bound_2 Σ `{diffprivRGpreS Σ} `{forall foo, @XOR_spec Σ foo _ _ xor_struct} σ σ' (Q : nat) :
  ...
  Lemma CPA_bound Σ `{diffprivRGpreS Σ} `{forall foo, @XOR_spec Σ foo _ _ xor_struct} σ σ' (Q : nat) :
...
Section implementation.
  ...
  #[local] Instance XOR_spec_mod `{!diffprivRGS Σ} : @xor.XOR_spec _ _ _ _ XOR_minus_mod := xor.XOR_spec_minus_mod bit.
