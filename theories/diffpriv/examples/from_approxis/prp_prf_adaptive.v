...
From clutch.diffpriv Require Import diffpriv map list prf prp sum_seq.
...
Section prp_prf.
  ...
  Lemma PRF_PRP_ARC Σ `{diffprivRGpreS Σ} σ σ' :
    DPcoupl
      (lim_exec (PRF, σ))
      (lim_exec (PRP, σ'))
      (=)
  ...
  Lemma PRP_PRF_ARC Σ `{diffprivRGpreS Σ} σ σ' :
    DPcoupl
      (lim_exec (PRP, σ))
      (lim_exec (PRF, σ'))
      (=)
  ...
  Corollary PRF_PRP_bound Σ `{diffprivRGpreS Σ} σ σ' :
    (((lim_exec (PRF, σ)) #true)
     <=
       ((lim_exec (PRP, σ')) #true) + ((Q-1) * Q / (2 * S Input)))%R.
  Proof.
    apply ARcoupl_eq_elim, DPcoupl_to_ARcoupl.
    by eapply PRF_PRP_ARC.
  Qed.

  Corollary PRP_PRF_bound Σ `{diffprivRGpreS Σ} σ σ' :
    (((lim_exec (PRP, σ)) #true)
     <=
...
Proof.
  apply Rabs_le.
  opose proof (PRF_PRP_bound K N adv typed Q diffprivRΣ σ' σ) as h => //.
  opose proof (PRP_PRF_bound K N adv typed Q diffprivRΣ σ σ') as h' => //.
  split ; lra.
Qed.
