From clutch.diffpriv Require Import diffpriv map list security_aux.
...
Class XOR_spec `{!diffprivRGS Σ} `{XOR} :=
...
Section xor_minus_mod.
  ...
  Lemma xor_minus_mod_correct_l `{!diffprivRGS Σ} E K (x : Z) (y : nat)
    (_: (0<=x)%Z)
  ...
  Lemma xor_minus_mod_correct_r  `{!diffprivRGS Σ} E K (x : Z) (y : nat)
    (_: (0<=x)%Z)
    (Hx : ((Z.to_nat x) < S Message2'))
    (Hy: y < S Message2') e A:
  ...
  Fact xor_minus_mod_sem_typed : forall `{!diffprivRGS Σ},
  ...
  #[local] Instance XOR_spec_minus_mod `{!diffprivRGS Σ} : @XOR_spec _ _ Output' Output' XOR_minus_mod.
  Proof.
    unshelve econstructor.
