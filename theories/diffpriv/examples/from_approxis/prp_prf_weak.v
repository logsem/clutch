From clutch.diffpriv Require Export diffpriv map list prf prp sum_seq.
...
Section Diffpriv.
  (* We derive the logical refinement in Diffpriv, in order to then conclude
     via adequacy. *)
...
End Diffpriv.
  ...
  Lemma ARC_wPRF_wPRP Σ `{diffprivGpreS Σ}
    σ σ' (Q : nat) (ε : R) :
    ((fold_left Nat.add (seq 0 Q) 0%nat / S val_size)%R = ε) →
    ARcoupl
      ...
      (=) ε.
  Proof.
    intros <-. unshelve eapply DPcoupl_to_ARcoupl, adequacy.wp_adequacy ; eauto. 1: lra.
    1: apply Rcomplements.Rdiv_le_0_compat; real_solver.
    iIntros (?) "spec ε δ".
    iApply (wp_wPRF_wPRP _ [] with "[spec $δ]") => //.
    iNext ; iIntros. iExists _. iFrame. done.
  Qed.

  Lemma ARC_wPRP_wPRF Σ `{diffprivGpreS Σ}
    σ σ' (Q : nat) (ε : R) :
    ((fold_left Nat.add (seq 0 Q) 0%nat / S val_size)%R = ε) →
    ARcoupl
      (lim_exec ((wPRP #false dummy_scheme #Q), σ))
  ...
  Proof.
    intros <-.
    unshelve eapply DPcoupl_to_ARcoupl, adequacy.wp_adequacy ; eauto. 1: lra.
    1: apply Rcomplements.Rdiv_le_0_compat; real_solver.
    iIntros (?) "spec ε δ".
    iApply (wp_wPRP_wPRF _ [] with "[spec $δ]") => //.
    iNext ; iIntros. iExists _. iFrame. done.
  Qed.

  Corollary wPRF_wPRP_bound Σ `{diffprivGpreS Σ} σ σ' (Q : nat) ε vres :
  ...
  Corollary wPRP_wPRF_bound Σ `{diffprivGpreS Σ} σ σ' (Q : nat) ε vres :
    ((((Q - 1) * Q) / (2 * S val_size)) = ε)%R →
