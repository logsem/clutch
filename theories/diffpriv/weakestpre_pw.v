        ...
        | None =>
            ...
            ∨
              (
                prog_coupl e1 σ2 e2' σ2' ε2 δ2 (λ e3 σ3 e3' σ3' ε3 δ3,
                ▷ spec_coupl ∅ σ3 e3' σ3' ε3 δ3 (λ σ4 e4' σ4' ε4 δ4,
                (* This should cover the case where Φ(v) = ∃ v', ⤇ fill K v' ∗ ⌜v = v'⌝ . *)
                (* (∀ K `{!LanguageCtx K} (v1 v1' : val Λ), spec_interp (K (of_val v1'), σ2') -∗ ⌜v1 = v1'⌝ -∗ Φ v1) ∗ *)
                (* Simplified version without context on the right. Let's try this first. *)
                (* (∀ (v1 v1' : val Λ), spec_interp ((of_val v1'), σ2') -∗ ⌜v1 = v1'⌝ -∗ Φ v1) ∗ *)
                (* (∀ (v1 v1' : val Λ), spec_interp ((of_val v1'), σ2') -∗ ⌜v1 = v1'⌝ = Φ v1) ∗ *)
                (□ (∀ v1, Φ v1 ∗-∗ ∃ v1', spec_interp ((of_val v1'), σ2') ∗ ⌜v1 = v1'⌝)) ∗
                (* Alternatively, could require this only when Φ occurs as postcondition; however,
                we'd probably prove this by monotonicity and the above assumption. *)
                (* (wp E e3 (λ v1, ∃ v1' : val Λ, spec_interp (of_val v1', σ2') ∗ ⌜v1 = v1'⌝) -∗ wp E e3 Φ) ∗ *)

                    |={∅, E}=> state_interp σ4 ∗ spec_interp (e4', σ4') ∗ err_interp ε4 δ4 ∗
                               ∀ RES, wp E e3
                                        (λ v1, ∃ v1' : val Λ, spec_interp (of_val v1', σ4') ∗ ⌜v1 = RES → v1' = RES⌝))))
      end))%I.
...
Proof.
  ...
  rewrite !/prog_coupl.
  do 44 f_equiv.
  all: do 4 f_equiv ; f_contractive.
  - apply least_fixpoint_ne_outer; [|done].
    intros ? [? ?]. rewrite /spec_coupl_pre.
    do 21 f_equiv.
  - apply least_fixpoint_ne_outer; [|done].
...
Proof.
  ...
  iApply spec_coupl_ret.
  iDestruct "H" as "[H|H]".
  - iLeft. iApply (prog_coupl_mono with "[HΦ Hclose] H").
    ...
    iModIntro.
    by iApply ("IH" with "[] H").
  - iRight. iApply (prog_coupl_mono with "[HΦ Hclose] H").
    iIntros (e2 σ3 e3' σ3' ε3 δ3) "H !>".
    iApply (spec_coupl_mono with "[HΦ Hclose] H"); [done|].
    iIntros (σ4 e4' σ4' ε4 δ4). iIntros "[#Heq H]".
    iSplitR.
    + iModIntro.
      iIntros (v1).
      iSpecialize ("Heq" $! v1).
...
Proof.
  ...
  iIntros (σ2 e2' σ2' ε2 δ2) "H".
  iDestruct "H" as "[H|H]".
  - iLeft. iApply (prog_coupl_mono with "[HR] H").
    ...
    iMod ("H" with "[$]").
    by iFrame.
  - iRight. iApply (prog_coupl_mono with "[HR] H").
    iIntros (e3 σ3 e3' σ3' ε3 δ3) "H !>".
    iApply (spec_coupl_mono with "[HR] H"); [done|].
    iIntros (σ4 e4' σ4' ε4 δ4) "[Heq H]".
    iSplitL "Heq" ; last first.
    + iMod "H" as "($ & $ & $ & H)".
      iMod "HR".
      iModIntro.
      iIntros (RES).
    ...
    + iIntros "H".
      ...
      iAssert (WP e3 @ s; E2 {{ v, P ={E1}=∗ Φ v }})%I with "[H Heq]" as "HH".
      {
        iApply "Heq".
        iApply (wp_strong_mono _ with "H") => //. 1: admit.
        iIntros (??????) "(? & ? & ? & H)".
        iApply spec_coupl_ret. iFrame.
        done.
      }
      (* iSpecialize ("Heq" with "H"). *)
      iApply (wp_strong_mono with "HH") => //.
      iIntros (??????) "(? & ? & ? & H)".
      iApply spec_coupl_ret. iFrame.
      iMod "H".
Qed.
...
Proof.
  ...
  - iRight.
    ...
    +
      ...
      (* iApply "IH".
         iApply "H".
         iSpecialize ("Φeq" with "H") => //. *)

      admit.
    + iMod "H" as "($ & $ & $ & H)".
      iModIntro. iIntros (RES).
      iSpecialize ("H" $! RES).
      iApply "IH".
      (* iApply "H". *)
      admit.
...
Proof.
  ...
  iApply spec_coupl_ret.
  iDestruct "H" as "[H|H]".
  - iLeft. iApply (prog_coupl_mono with "[] H").
    iIntros (e2 σ3 e3' σ3' ε3 δ3) "H !>".
    iApply (spec_coupl_mono with "[] H"); [done|].
    iIntros (σ4 e4' σ4' ε4 δ4) "> ($ & $ & $ & H)".
    iApply ("IH" with "H").
  - iRight. iApply (prog_coupl_mono with "[] H").
    iIntros (e2 σ3 e3' σ3' ε3 δ3) "H !>".
    iApply (spec_coupl_mono with "[] H"); [done|].
    iIntros (σ4 e4' σ4' ε4 δ4). iIntros "[Heq H]". iSplitL "Heq".
    + admit.
    + iIntros "> ($ & $ & $ & H)".
      iApply ("IH" with "H").
Qed.
