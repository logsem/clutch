...
Section rules.
  ...
  (** helper lemma  *)
  Lemma Mcoupl_steps_ctx_bind_r `{Countable A} (μ : distr A)
    e1' σ1' R (ε : nonnegreal) K :
    to_val e1' = None →
    Mcoupl μ (prim_step e1' σ1') R ε →
    Mcoupl μ (prim_step (fill K e1') σ1')
      (λ a '(e2', σ2'), ∃ e2'', (e2', σ2') = (fill K e2'', σ2') ∧ R a (e2'', σ2')) ε.
  Proof.
    intros Hcpl Hv.
    rewrite fill_dmap //= -(dret_id_right μ ) /=.
    eapply (Mcoupl_dbind' ε 0) ; [lra|..|done].
    intros ? [] ?.
    apply Mcoupl_dret=>/=; [done|]. eauto.
  Qed.

  Lemma Mcoupl_laplace_step
    (loc loc' k k' : Z)
    ...
    0 < IZR num / IZR den →
    ε' = (IZR k' * ε) →
    Mcoupl (language.prim_step (Laplace #num #den #loc) σ1)
