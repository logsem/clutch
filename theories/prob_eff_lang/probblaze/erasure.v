...
From clutch.common Require Import exec language erasable.
From clutch.prob_eff_lang.probblaze Require Import notation syntax semantics metatheory.
...
Proof.
  ...
  rewrite !lim_exec_unfold. rewrite /dbind_pmf.
  rewrite Haux.
...
Lemma refRcoupl_erasure_erasable (e1 e1' : expr) σ1 σ1' μ1 μ2 R Φ n :
  ...
  erasable μ1 σ1->
  erasable μ2 σ1'->
  (∀ σ2 σ2' : language.state blaze_prob_lang, R σ2 σ2' → refRcoupl (exec (S n) (e1, σ2)) (lim_exec (e1', σ2')) Φ) ->
...
Lemma ARcoupl_erasure_erasable (e1 e1' : expr) ε ε1 ε2 σ1 σ1' μ1 μ2 R Φ n :
  ...
  erasable μ1 σ1->
  erasable μ2 σ1'->
  (∀ σ2 σ2' : language.state blaze_prob_lang, R σ2 σ2' → ARcoupl (exec n (e1, σ2)) (lim_exec (e1', σ2')) Φ ε2) ->
