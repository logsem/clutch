...
Proof with (repeat foldkont) using G.
  ...
  simpl. iIntros (autheff_l autheff_r) "Hautheff".
  simpl. unfold sem_ty_prod.
  iDestruct "Hautheff" as (asnd_l asend_r arcv_l arcv_r) "Hautheff".
  iDestruct "Hautheff" as "(%Hautheff_l & (%Hautheff_r & Hautheffrel))".
  rewrite Hautheff_l.
  rewrite Hautheff_r.
  brel_pures. iModIntro.
  iIntros (keyeff_l keyeff_r) "Hkeyeff".
  iDestruct "Hkeyeff" as (kysnd_l kysend_r kyrcv_l kyrcv_r) "Hkeyeff".
  ...
  set (kl2 := (match: "p" with
       ...
       | InjR <> =>
         ...
         match: "r" with
           InjL <> => "k" (InjLV #()%V)
         | InjR "w" => kysnd_l alice;; "k" (InjR (vgval (g ^+ n)))
         end
       end)%E).
  set (kl3 := ( match: "payload" with
         InjL "payload" =>
           ...
           match: ! #l_auth with
             InjL <> =>
               #l_auth <- InjR "m";; asnd_l ("m", "dst");; "k" #()%V
           | InjR "message" => "k" #()%V
           end
       | InjR "from" =>
           ...
           | InjR "x" =>
             match: ! #l_sim with
               InjL <> =>
                 let: "m'" := #()%V;; rand(#lbl:α) #(S n'') in
                 let: "mA" := vexp g "m'" in
                 #l_sim <- InjR "m'";; asend_r ("mA", bob);; "k" #()%V
             | InjR "m" => "k" #()%V
             end
           end
       | InjR "from" =>
         kysend_r "from";; 
         let: "r" := kyrcv_r "from" in
         match: "r" with
           InjL <> => "k" (InjLV #()%V)
         | InjR "x" =>
           let: "rla" := arcv_r "from" in
           match: "rla" with
             InjL <> => "k" (InjLV #()%V)
           | InjR "x" => "k" ! #l_sim
           end
