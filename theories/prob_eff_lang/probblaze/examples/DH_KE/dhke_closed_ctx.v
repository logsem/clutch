From clutch.prob_eff_lang.probblaze.examples.DH_KE Require Import advantage_dhke_lazy valgroup_Zpx.
From clutch.prob_eff_lang.probblaze Require Import adequacy syntax.
From clutch Require Import stdpp_ext.
From iris.base_logic.lib Require Import ghost_map.
From iris.algebra Require Import excl dfrac_agree.

#[local] Definition rfc3526_id18 : nat :=
  .

#[local] Definition p''' := (rfc3526_id18 - 3).
#[local] Definition p := S (S (S p''')).

Fact addSC p m : p = m+3 -> S (S (S (p - 3))) = p.
Proof. intros ->. rewrite PeanoNat.Nat.add_sub. rewrite PeanoNat.Nat.add_comm. reflexivity. Qed.
Fact p_eq_rfc3526_id18 : p = rfc3526_id18.
  unfold p, p'''.
  eapply (addSC _ (rfc3526_id18 - 3)).
  rewrite PeanoNat.Nat.sub_add; first reflexivity.
  eapply PeanoNat.Nat.le_trans.
  - eapply PeanoNat.Nat.le_add_r.
  - eapply PeanoNat.Nat.le_refl.
Qed.

Section multiplicative.
  Import valgroup_Zpx.


  Context `{rfc3526_id18_prime : is_true (prime.prime (rfc3526_id18))}.

  Fact p_prime : is_true (prime.prime p).
  Proof using rfc3526_id18_prime. rewrite p_eq_rfc3526_id18. exact rfc3526_id18_prime. Qed.

  Definition closedΣ : gFunctors := #[GFunctor (exclR unitO); GFunctor dfracR; GFunctor (dfrac_agreeR valO); probblazeRΣ].
  Local Instance subG_probblazeRGPreS {Σ} : subG closedΣ Σ → probblazeRGpreS Σ.
  Proof. solve_inG. Qed.
  Local Instance inGexcl : inG closedΣ (exclR unitO).
  Proof. apply (subG_inG _ (GFunctor (exclR unitO))). apply _. Qed.
  Local Instance inGdfrac : inG closedΣ dfracR.
  Proof. apply (subG_inG _ (GFunctor (dfracR))). apply _. Qed.
  Local Instance inGdfrac_agree : inG closedΣ (dfrac_agreeR valO).
  Proof. apply (subG_inG _ (GFunctor (dfrac_agreeR valO))). apply _. Qed.

  Definition DHKE_secure := @adv_DHKE_typed (vg_p _) (cgs_p p''') (@vgg_p p''' rfc3526_id18_prime) (@cg_p _ p_prime) closedΣ _ _ _ _.  

  Set Printing Implicit.
  Check DHKE_secure.

End multiplicative.

