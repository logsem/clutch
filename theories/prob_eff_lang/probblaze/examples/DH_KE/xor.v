...
Class XOR {Key Support : nat} :=
  { xor : val
  ; xor_sem : nat → nat → nat
  ; xor_bij (x : nat) : Bij (xor_sem x)
  ; xor_dom (x : nat) : x < S Key -> (∀ n : nat, n < S Support → xor_sem x n < S Support)
...
Class XOR_spec `{!probblazeRGS Σ} `{XOR} :=
  { XOR_CORRECT_L := ∀ E K (x : nat) (y : nat) e X R
                       ...
                       (_: (x < S Key))
                       (_: y < S Support),
      (BREL (fill K (of_val #(xor_sem (x) (y)))) ≤ e @ E <|X|> {{R}})
      -∗ BREL (fill K (xor #x #y)) ≤ e @ E <|X|> {{R}}
  ; XOR_CORRECT_R := ∀ E K (x : nat) (y : nat) e X R
                       (*(_: (0<=x)%Z)*)
                       (_: (x < S Key))
                       (_: y < S Support),
      (BREL e ≤ (fill K (of_val #(xor_sem (x) (y)))) @ E <|X|> {{R}})
      -∗ BREL e ≤ (fill K (xor #x #y)) @ E <|X|> {{R}}
