...
Lemma REAL_CHAN_CHAN_SIM_F_CHAN :
  ⊢ sem_val_typed (REAL_CHAN)%V (λ: "f", CHAN_SIM_lazy (F_CHAN "f"))%V                                                                                         (∀ᵣ θ__L ,(∀ᵣ θₕ, (((𝔾 -{ θₕ }->  𝟙) × (𝟙 -{ θₕ }-> (Option 𝔾))) -{ sem_row_union  θₕ θ__L }-∘ 𝟙)) ⊸ (*type of client*)
...
Proof using Bdd_int_vg Bij_log Bij_xor_sem G H
