...
Section adv_sc.
  ...
  Qed.
    ...
    (∀ `{!probblazeRGS Σ},⊢ sem_val_typed A A (τ_sc → 𝔹)%T) →
    advantage A (λ: "f" "effs", (reduction DH_real) (λ: "doCRS", OT_Real_Sender_corrupt "f" ("effs", "doCRS")))%V OT_SIM_FOT_thunk #true <= 1/n. 
  Proof using n_prime G inG2 H. 
    intros. eapply sem_typed_advantage_aff; try done; last split.
    - apply Rcomplements.Rdiv_le_0_compat; first lra.
      apply lt_0_INR. lia.
  ...
  Theorem adv_ot A :
    ...
    <= advantage A 
         (λ: "f" "effs", (reduction DH_rand) (λ: "doCRS", OT_Real_Sender_corrupt "f" ("effs", "doCRS")))%V 
         (λ: "f" "effs", (reduction DH_real) (λ: "doCRS", OT_Real_Sender_corrupt "f" ("effs", "doCRS")))%V #true + 2/n.
  Proof using n_prime G inG2 H.
