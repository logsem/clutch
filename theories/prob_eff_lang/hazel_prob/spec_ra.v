...
Class specG_prob_eff_lang Σ := SpecGS {
  #[local] specG_prob_eff_lang_prog_inG :: inG Σ (authR progUR);
  specG_prob_eff_lang_prog_name : gname;

  #[local] specG_prob_eff_lang_heap :: ghost_mapG Σ loc val;
  #[local] specG_prob_eff_lang_tapes :: ghost_mapG Σ loc tape;

  specG_prob_eff_lang_heap_name : gname;
  specG_prob_eff_lang_tapes_name : gname;                      
}.
...
Section resources.
  Context `{!specG_prob_eff_lang Σ}.
  Definition spec_prog_auth e :=
    own specG_prob_eff_lang_prog_name (● (Excl' e : progUR)).
  Definition spec_heap_auth `{specG_prob_eff_lang Σ} :=
    @ghost_map_auth _ _ _ _ _ specG_prob_eff_lang_heap specG_prob_eff_lang_heap_name 1.
  Definition spec_tapes_auth `{specG_prob_eff_lang Σ} :=
    @ghost_map_auth _ _ _ _ _ specG_prob_eff_lang_tapes specG_prob_eff_lang_tapes_name 1.
  ...
  Definition spec_heap_frag (l : loc) v dq: iProp Σ :=
    (@ghost_map_elem _ _ _ _ _ specG_prob_eff_lang_heap specG_prob_eff_lang_heap_name l dq v).

  Definition spec_tapes_frag (l : loc) v dq: iProp Σ :=
    (@ghost_map_elem _ _ _ _ _ specG_prob_eff_lang_tapes specG_prob_eff_lang_tapes_name l dq v).
End resources.
...
(** Spec heap *)
Notation "l ↦ₛ{ dq } v" := (@ghost_map_elem _ _ _ _ _ specG_prob_eff_lang_heap specG_prob_eff_lang_heap_name l dq v)
...
(** Spec tapes *)
Notation "l ↪ₛ{ dq } v" := (@ghost_map_elem _ _ _ _ _ specG_prob_eff_lang_tapes specG_prob_eff_lang_tapes_name l dq v)
  (at level 20, format "l  ↪ₛ{ dq }  v") : bi_scope.
Notation "l ↪ₛ□ v" := (l ↪ₛ{ DfracDiscarded } v)%I
  (at level 20, format "l  ↪ₛ□  v") : bi_scope.
Notation "l ↪ₛ{# q } v" := (l ↪ₛ{ DfracOwn q } v)%I
...
Definition nat_spec_tape `{specG_prob_eff_lang Σ} l (N : nat) (ns : list nat) : iProp Σ :=
...
#[global] Instance spec_rules_spec_updateGS `{!specG_prob_eff_lang Σ} :
  spec_updateGS (lang_markov eff_prob_lang) Σ := Spec_updateGS spec_auth.
