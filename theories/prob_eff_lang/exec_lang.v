...
Lemma stepN_det_step_ctx (K : ectx) n ρ (e1 e2 : expr) (σ1 σ2 : state) :
  prim_step e1 σ1 (e2, σ2) = 1%R →
...
Proof.
  ...
  rewrite -fill_step_prob //.
  { eapply (val_stuck _ σ1 (e2, σ2)). rewrite H. lra. }
  { eapply (eff_prim_stuck _ σ1 (e2, σ2)). rewrite H. lra. }
Qed.

Lemma stepN_PureExec_ctx (K : ectx) (P : Prop) m n ρ (e e' : expr) σ :
  P →
  PureExec P n e e' →
...
Proof.
  ...
  specialize (IHn _ _ m Hsteps Hdet).
  rewrite -plus_n_Sm.
  eapply stepN_det_step_ctx; [|done].
  apply Hpstep.
Qed.
