...
Structure conEctxLanguage := ConEctxLanguage {
  expr : Type;
  val : Type;
  ectx : Type;
  state : Type;
...
Global Arguments ConEctxLanguage {_ _ _ _ _ _ _ _ _ _ _ _ _ _ _ _ _ _ _ _ _ _} _.
...
Section con_ectx_language.
  Context {Λ : conEctxLanguage}.
  ...
  Proof. apply con_ectx_language_mixin. Qed.
  ...
  Lemma fill_lift_comp' (K1 K2 : ectx Λ) :
    fill_lift' (comp_ectx K1 K2) = fill_lift' K1 ∘ fill_lift' K2.
  Proof.
    extensionality ρ. destruct ρ as [[]].
    rewrite /fill_lift'/= -fill_comp //=.
  Qed.
  ...
  Instance inj_fill' (K : ectx Λ) : Inj eq eq (fill_lift' K).
  Proof. intros [[]] [[]]. rewrite /fill_lift'.
         intros. by simplify_eq.
  Qed.

  Definition prim_step (e1 : expr Λ) (σ1 : state Λ) : distr (expr Λ * state Λ * list (expr Λ)) :=
  ...
  Definition con_ectx_lang_mixin : ConLanguageMixin (@of_val Λ) to_val prim_step state_step get_active.
  Proof.
    split.
    - apply con_ectx_language_mixin.
    - apply con_ectx_language_mixin.
  ...
  Proof.
    ...
    rewrite -dmap_comp.
    replace (fill K e2, σ2, efs) with (fill_lift' K (e2, σ2, efs)); [|done].
    rewrite (dmap_elem_eq (dmap _ _) (e2, σ2, efs)) //.
  Qed.
  ...
  Lemma prim_step_iff e1 e2 σ1 σ2 efs :
    prim_step e1 σ1 (e2, σ2, efs) > 0 ↔
    ∃ K e1' e2',
      fill K e1' = e1 ∧
      fill K e2' = e2 ∧
      head_step e1' σ1 (e2', σ2, efs) > 0.
  Proof.
    split.
  ...
  Proof.
    ...
    eexists (fill (comp_ectx K K') e2', σ2, _).
  ...
  Proof.
    intros Hatomic Hsub σ e' σ' efs (K & e1' & e2' & <- & <- & Hs)%prim_step_iff.
    assert (K = empty_ectx) as -> by eauto 10 using val_head_stuck.
  ...
  Lemma head_reducible_prim_step_ctx K e1 σ1 e2 σ2 efs:
    head_reducible e1 σ1 →
    prim_step (fill K e1) σ1 (e2, σ2, efs) > 0 →
    ∃ e2', e2 = fill K e2' ∧ head_step e1 σ1 (e2', σ2, efs) > 0.
  Proof.
  ...
  Global Instance con_ectx_lang_ctx K : ConLanguageCtx (fill K).
  Proof.
    split; simpl.
...
Global Arguments ConLanguageOfEctx : simpl never.
