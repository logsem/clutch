...
Section defs.
  ...
  Section proofs.
    ...
    Lemma xor_correct_l E K (x : Z) (y : fin (S Message)) (Hx : ((Z.to_nat x) < S Message)) e A :
      (REL (fill K (of_val #(xor_sem (nat_to_fin Hx) (y)))) << e @ E : A)
      -∗ REL (fill K (xor #x #y)) << e @ E : A.
    Admitted.
    
    Lemma xor_correct_r E K (x : Z) (y : fin (S Message)) (Hx : ((Z.to_nat x) < S Message)) e A :
