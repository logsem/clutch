...
Section rules.
  ...
  Proof.
    ...
    { apply nnreal_ext; simpl; lra. }
    iApply prog_coupl_step_l_err; [done|solve_red|..].
  ...
  Proof.
    ...
    replace ε_now with (nnreal_plus ε ε'); last first.
    { apply nnreal_ext; simpl; lra. }
    set R := (λ σ2 ρ2', ∃ n : fin (S N), σ2 = σ1 ∧ ρ2' = (fill K #n, σ1') ∧ n ≠ m).
    iApply (spec_coupl_erasable_steps 1 R (dret σ1)); [done|..].
    { rewrite pexec_1 step_or_final_no_final //; last first.
      { apply reducible_not_final. solve_red. }
      rewrite /= fill_dmap //.
      rewrite -(dret_id_right (dret σ1)).
      eapply (ARcoupl_dbind' ε 0%NNR); [done|done|simpl; lra|..]; last first.
      { apply (ARcoupl_rand_no_coll_r _ m); [|done|].
        - apply lt_0_INR. lia.
        - rewrite Nz //. }
      simpl. intros σ2 [e2' σ2'] (n & -> & [=-> ->] & ?).
      apply ARcoupl_dret. rewrite /R; eauto. }
    { apply dret_erasable. }
    iIntros (??? (?&->&[=-> ->]&?)) "!>".
  ...
  Lemma wp_couple_rand_rand_inj (N M : nat) (f: nat → nat) z w K E Φ (ε : nonnegreal) :
    ...
    ⤇ fill K (rand #w) ∗
    ↯ ε ∗
    ▷ (∀ (n : fin (S N)), ⤇ fill K #(f n) -∗ WP (Val #n) @ E {{ Φ }})
    ⊢ WP rand #z @ E {{ Φ }}.
  Proof.
    iIntros (Hdom Hinj).
    ...
    { apply nnreal_ext; simpl; lra. }
    set (R := (λ '(e2, σ2) '(e2', σ2'),
        ∃ (n : fin _), (e2, σ2) = (Val #n, σ1) ∧ (e2', σ2') = (fill K #(g n), σ1'))).
    iApply (prog_coupl_steps _ _ _ R);
    ...
    { rewrite fill_dmap //= -(dret_id_right (prim_step _ _)) /=.
      ...
      { eapply (ARcoupl_rand_rand_inj _ _ g); auto. }
      intros [] [] (n & [=-> ->] & [=-> ->]).
      apply ARcoupl_dret. simpl. eauto. }
  ...
  Proof.
    iIntros (-> ->  HNM Hε) "(Hr & Hε & Hwp)".
    iApply (wp_couple_rand_rand_inj _ _ (λ x, x)); [|lia|done|done|..].
    ...
    { iPureIntro. by rewrite fin_to_nat_to_fin. }
