...
From clutch.paris Require Import primitive_laws model app_rel_rules proofmode.

(** * General-purpose tactics *)
Lemma tac_rel_bind_l `{!parisRGS Σ} e' K ℶ E e t A :
  e = fill K e' →
  envs_entails ℶ (REL fill K e' << t @ E : A) →
  envs_entails ℶ (REL e << t @ E : A).
Proof. intros. subst. assumption. Qed.

Lemma tac_rel_bind_r `{!parisRGS Σ} (t' : expr) K ℶ E e t A :
  t = fill K t' →
...
Lemma tac_rel_pure_l `{!parisRGS Σ} K e1 ℶ ℶ' E e e2 eres ϕ n m t A :
  e = fill K e1 →
  PureExec ϕ n e1 e2 →
  ϕ →
  m = n ∨ m = 0 →
...
Lemma tac_rel_load_l_simp `{!parisRGS Σ} K ℶ1 ℶ2 i1 p (l : loc) q v e t eres A E :
  e = fill K (Load (# l)) →
...
(** Store *)
Lemma tac_rel_store_l_simpl `{!parisRGS Σ} K ℶ1 ℶ2 ℶ3 i1 (l : loc) v e' v' e t eres A E :
  e = fill K (Store (# l) e') →
  IntoVal e' v' →
  MaybeIntoLaterNEnvs 1 ℶ1 ℶ2 →
...
Lemma tac_rel_alloc_l_simpl `{!parisRGS Σ} K ℶ1 ℶ2 e t e' v' A E :
  e = fill K (Alloc e') →
  IntoVal e' v' →
  MaybeIntoLaterNEnvs 1 ℶ1 ℶ2 →
...
Lemma tac_rel_alloctape_l_simpl `{!parisRGS Σ} K ℶ1 ℶ2 e N z t A E :
  TCEq N (Z.to_nat z) →
...
Lemma tac_rel_rand_l `{!parisRGS Σ} K ℶ1 ℶ2 i1 (α : loc) N (z : Z) n ns e t tres A E :
