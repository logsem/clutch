...
From clutch.micrometer Require Import primitive_laws model app_rel_rules proofmode.

(*

(** * General-purpose tactics *)
Lemma tac_rel_bind_l `{!approxisRGS Σ} e' K ℶ E e t A :
  e = fill K e' →
  envs_entails ℶ (REL fill K e' << t @ E : A) →
  envs_entails ℶ (REL e << t @ E : A).
Proof. intros. subst. assumption. Qed.

Lemma tac_rel_bind_r `{!approxisRGS Σ} (t' : expr) K ℶ E e t A :
  t = fill K t' →
...
Lemma tac_rel_pure_l `{!approxisRGS Σ} K e1 ℶ ℶ' E e e2 eres ϕ n m t A :
  e = fill K e1 →
  PureExec ϕ n e1 e2 →
  ϕ →
  m = n ∨ m = 0 →
...
Lemma tac_rel_pure_r `{!approxisRGS Σ} K e1 ℶ E (e e2 eres : expr) ϕ n t A :
  e = fill K e1 →
  PureExec ϕ n e1 e2 →
  ϕ →
  eres = fill K e2 →
...
Lemma tac_rel_load_l_simp `{!approxisRGS Σ} K ℶ1 ℶ2 i1 p (l : loc) q v e t eres A E :
  e = fill K (Load (# l)) →
...
Lemma tac_rel_load_r `{!approxisRGS Σ} K ℶ1 E i1 p (l : loc) q e t tres A v :
  t = fill K (Load (# l)) →
...
(** Store *)
Lemma tac_rel_store_l_simpl `{!approxisRGS Σ} K ℶ1 ℶ2 ℶ3 i1 (l : loc) v e' v' e t eres A E :
  e = fill K (Store (# l) e') →
  IntoVal e' v' →
  MaybeIntoLaterNEnvs 1 ℶ1 ℶ2 →
...
Lemma tac_rel_store_r `{!approxisRGS Σ} K ℶ1 ℶ2 i1 E (l : loc) v e' v' e t eres A :
  e = fill K (Store (# l) e') →
  IntoVal e' v' →
...
Lemma tac_rel_alloc_l_simpl `{!approxisRGS Σ} K ℶ1 ℶ2 e t e' v' A E :
  e = fill K (Alloc e') →
  IntoVal e' v' →
  MaybeIntoLaterNEnvs 1 ℶ1 ℶ2 →
...
Lemma tac_rel_alloc_r `{!approxisRGS Σ} K' ℶ E t' v' e t A :
  t = fill K' (Alloc t') →
  IntoVal t' v' →
...
Lemma tac_rel_alloctape_l_simpl `{!approxisRGS Σ} K ℶ1 ℶ2 e N z t A E :
  TCEq N (Z.to_nat z) →
