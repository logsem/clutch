...
From clutch.diffpriv_simple_pw_again Require Import weakestpre_simple.
  ...
  (match to_val e1 with
   ...
   | None =>
       ∀ σ1 e1' σ1' ε δ,
         ...
         ⌜reducible (e1, σ1)⌝ ∗
         ∃ (R : cfg  → cfg  → Prop) (ε1 ε2 δ1 δ2 : nonnegreal) (n : nat),
...
Lemma wp_lift_pure_step `{!Inhabited (state)} E E' Φ e1 s :
  (∀ σ1, reducible (e1, σ1)) →
...
Lemma wp_lift_pure_det_step `{!Inhabited (state)} {E E' Φ} e1 e2 s :
  (∀ σ1, reducible (e1, σ1)) →
...
Lemma wp_pure_step_fupd `{!Inhabited (state)} E E' e1 e2 φ n Φ s :
  PureExec φ n e1 e2 →
  φ →
...
Lemma wp_pure_step_later `{!Inhabited (state)} E e1 e2 φ n Φ s :
  PureExec φ n e1 e2 →
  φ →
