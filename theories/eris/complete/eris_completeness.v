...
From clutch.eris.complete Require Export ectx_lang_completeness lang_completeness no_alloc.
...
Proof.
  ...
  *) 
  ...
  (* rand on unmatching tape (same as usual rand)*) 
  {
    ...
    wp_apply (wp_rand_exp_fin_tape_mismatch _ _ _ _ _ _ _ (λ n0 : fin (S (Z.to_nat z)), ε1 (Val #n0, σ)) with "H"); eauto.
    - apply Req_le.
      rewrite Hε1 Expval_dmap //=; first last.
      { apply ex_seriesC_finite. }
      apply SeriesC_ext => n0. 
    ...
    - iIntros (n0) "[Herr Hl]". 
      ...
      rewrite insert_id //=.
      iCombine "Hheap Htapes" as "Hheap".
      iMod ("Hind" $! (Val #n0) with "[] Hheap") as "Hind".
    + iPureIntro. rewrite head_prim_step_eq /ectx_language.head_step /prob_lang.head_step //= Hσl.
