...
Module Type BernoulliSpec (T : BernoulliTapeSpec).
  Import T.

  Context `{!erisGS Σ}.

  Parameter bernoulli_tape : val.

  Notation bernoulli := (bernoulli_tape #())%E.

  Parameter twp_bernoulli_scale :
    ∀ (N M : nat) (ε ε1 ε2 : R) 
      (p := N / S M),
    N ≤ S M →
    0 <= ε1 →
  ...
  (* TODO: See to add to a different module ? *)
  Parameter own_bernoulli_tape :
    loc → nat → nat → list (fin 2) → iPropI Σ.
  
  Parameter twp_presample_bernoulli : 
  ...
  Parameter twp_presample_bernoulli_adv_comp : 
    ∀ (e : expr) (α : loc) (Φ : val → iProp Σ) 
  ...
  Parameter twp_presample_bernoulli_planner :
...
Module BernoulliSpecLemmas (T : BernoulliTapeSpec) (M : BernoulliSpec(T)).
  Import M.
