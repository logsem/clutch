...
Qed.
  ...
  Proof.
    intros ??.
    split.
    { apply Rmult_le_pos; [apply Rmult_le_pos; [apply Rmult_le_pos; [apply RealDecrTrial_μnn; OK|apply Iverson_nonneg]|apply NegExp_ρ_nn]|apply Hbound]. }
    { rewrite /B/BUB.
      apply Rmult_le_compat.
      2: apply Hbound.
      3: apply Hbound.
      { apply Rmult_le_pos; [apply Rmult_le_pos; [apply RealDecrTrial_μnn; OK|apply Iverson_nonneg]|apply NegExp_ρ_nn].
      }
      ...
      apply Rmult_le_compat; [apply RealDecrTrial_μnn; OK|apply Iverson_nonneg|apply RealDecrTrial_μ_ub; OK|apply Iverson_le_1].
    }
  Qed.
  ...
  Lemma B_nn {F L M} (Hbound : ∀ n x, 0 <= x <= 1 → 0 <= F n x <= M) :
    ∀ n n0 : nat, ∀ x x0 : R, 0 <= x <= 1 → 0 <= x0 <= 1 → 0 <= B F L x n n0 x0.
  Proof.
    intros ??????.
    rewrite /B.
  ...
  Lemma B_PCts_y {F L} (HPcts : ∀ x1, PCts (F x1) 0 1) :
    ∀ n : nat, ∀ x x0, PCts (λ x1 : R, B F L x n x0 x1) 0 1.
  Proof.
    intros ???.
  ...
  Lemma B_PCts_x {F L} (HPcts : ∀ x1, PCts (F x1) 0 1) :
    ∀ n : nat, ∀ k x, PCts (λ x0 : R, B F L x0 n k x) 0 1.
  Proof.
    intros ???.
    apply PCts_mult; [apply PCts_mult; [apply PCts_mult|]|].
  ...
  Lemma B_PCts2_swap {F L n k} (HPcts : ∀ x1, PCts (F x1) 0 1) :
    PCts2 (λ x y : R, B F L y n k x) 0 1 0 1.
  Proof.
    apply PCts2_mult; [apply PCts2_mult; [apply PCts2_mult|]|].
    { apply PCts_const_y. apply RealDecrTrial_μ_PCts. }
    { apply PCts_const_y. PCts_auto_derive. }
    { apply PCts_const_x. apply NegExp_ρ_PCts. }
    { apply PCts_const_x. apply HPcts. }
  Qed.

  Lemma B_ub {F L M} (Hbound : ∀ n x, 0 <= x <= 1 → 0 <= F n x <= M) :
    ∀ x n n0 t, 0 <= x <= 1 → 0 <= t <= 1 → B F L x n n0 t <= 1 * 1 * exp (- (n0 - (L + 1))%nat) * M.
  Proof.
    intros ??????.
    rewrite /B.
    apply Rmult_le_compat.
    ...
    { apply NegExp_ρ_nn. }
    2: { apply NegExp_ρ_ub. }
    apply Rmult_le_compat; [apply RealDecrTrial_μnn; OK|apply Iverson_nonneg|apply RealDecrTrial_μ_le_1; OK|apply Iverson_le_1].
  Qed.

  Lemma ex_seriesC_exp_shifted (K : nat) :
    ex_seriesC (λ x0 : nat, exp (- (x0 - K)%nat)).
  Proof.
    apply (ex_SeriesC_nat_shiftN_r K).
    rewrite /compose//=.
    replace (λ x0 : nat, exp (- (x0 + K - K)%nat))
  ...
  Local Lemma ex_seriesC_BUB_scal (a b : R) :
    ex_seriesC (λ n : nat, 1 / fact n * a * b).
  Proof.
    apply ex_seriesC_scal_r. apply ex_seriesC_scal_r.
    setoid_rewrite Rdiv_def. apply ex_seriesC_scal_l.
    rewrite -ex_seriesC_nat. apply ex_exp_series.
  Qed.
  ...
  Proof.
    ...
    { intros ???.
      ...
      { apply RInt_ge_0; OK.
        ...
        rewrite /B.
        apply ex_RInt_mult; [apply ex_RInt_mult; [apply ex_RInt_mult; [apply RealDecrTrial_μ_ex_RInt|apply ex_RInt_const]|apply ex_RInt_const]|apply ex_RInt_const].
      }
      split; OK.
    ...
    intros n.
    apply Fubini_Step_ex_x, (B_PCts2_swap HPcts).
  Qed.


  Lemma QuadExists6 M {F L} {n} (HPcts : ∀ x1, PCts (F x1) 0 1) (Hbound : ∀ n x, 0 <= x <= 1 → 0 <= F n x <= M) :
    ex_RInt (λ x1 : R, SeriesC (λ x : nat, RInt (λ x0 : R, B F L x1 n x x0) 0 1)) 0 1.
  Proof.
    have L3 : ∀ n n0 : nat, PCts2 (λ x y : R, B F L x n n0 y) 0 1 0 1
      := fun _ _ => B_PCts2 HPcts.
    have L4 := B_PCts_y HPcts.

    have L5 := @B_nn _ L _ Hbound.
    have L6 := @B_ub _ L _ Hbound.
    ...
    { intros ???.
      apply SeriesC_ge_0'.
      intros ?.
      apply RInt_ge_0; OK; [apply PCts_RInt, L4|intros ??; apply L5; OK].
    }
      eapply (ex_RInt_SeriesC (fun n0 => ((* 1 / fact n*) 1 * 1 * exp (- (n0 - (L + 1))%nat) * M))); OK.
      { rewrite ex_seriesC_nat.
        apply ex_seriesC_scal_r.
        apply ex_seriesC_scal_l.
        apply ex_seriesC_exp_shifted.
      }
      { intros ???.
        split.
        { apply RInt_ge_0; OK.
    ...
    { intros ?.
      ...
      { apply (ex_seriesC_le _ (λ n0 : nat,  (1 / fact n0) * 1 * exp (- (n - (L + 1))%nat) * M)).
        ...
        { apply RInt_ge_0; OK.
          { apply ex_RInt_mult; [apply ex_RInt_mult; [apply ex_RInt_mult; [apply RealDecrTrial_μ_ex_RInt|apply ex_RInt_const]|apply ex_RInt_const]|apply ex_RInt_const]. }
          intros ??.
          apply Rmult_le_pos; [apply Rmult_le_pos; [apply Rmult_le_pos; [apply RealDecrTrial_μnn; OK|apply Iverson_nonneg]|apply Rexp_nn]|OK].
        }
        split; OK.
        rewrite -(Rabs_right _ (Rle_ge _ _ HH)).
        etrans.
  ...
  Proof.
    apply (ex_RInt_SeriesC (fun n => (1 / fact n) * 1 *  SeriesC (λ x0 : nat, RInt (λ x1 : R, NegExp_ρ (L + 1) x0 x1 * F x0 x1) 0 1))); OK.
    { rewrite ex_seriesC_nat.
      apply ex_seriesC_BUB_scal.
    }
    { intros ???.
      replace (λ k : nat, RInt (λ x0 : R, RealDecrTrial_μ x 0 n * Iverson (not ∘ Zeven) n * NegExp_ρ (L + 1) k x0 * F k x0) 0 1)
         with (λ k : nat, RInt (λ x0 : R, (RealDecrTrial_μ x 0 n * Iverson (not ∘ Zeven) n) * NegExp_ρ (L + 1) k x0 * F k x0) 0 1).
      ...
      have L2 : ∀ x0, 0 <= RInt (λ x1 : R, NegExp_ρ (L + 1) x0 x1 * F x0 x1) 0 1.
      { intros ?.
        apply RInt_ge_0; OK.
        { apply ex_RInt_mult.
          { apply PCts_RInt, NegExp_ρ_PCts. }
...
Qed.
  ...
  Proof.
    ...
    have L3 : ∀ a b, PCts2 (λ x y : R, B F L y a b x) 0 1 0 1
      := fun _ _ => B_PCts2_swap HPcts.

    have L4 := B_PCts_x HPcts.
  ...
  Proof.
    ...
    2: {
      ...
      { apply Rmult_le_compat.
        2: apply Hbound; OK.
        3: apply Hbound; OK.
        { apply Rmult_le_pos; [apply RealDecrTrial_μ0nn; OK|apply Iverson_nonneg].
        }
        apply Rmult_le_compat.
        { apply RealDecrTrial_μ0nn; OK. }
        { apply Iverson_nonneg. }
    ...
    { intros ?.
      ...
      apply ex_RInt_Rmult'. apply PCts_RInt. apply RealDecrTrial_μ0_PCts.
    }
  Qed.
  ...
  Proof.
    rewrite (RInt_ext _ (λ x : R, SeriesC (λ n : nat, RealDecrTrial_μ0 x n * Iverson Zeven n * F L x))).
    2: { intros ??. apply SeriesC_ext. intros ?. by rewrite RealDecrTrial_μ_base. }
