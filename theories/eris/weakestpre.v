...
Section glm.
  ...
  Lemma glm_adv_comp' e1 σ1 Z (ε : nonnegreal) :
      (∃ R (μ : distr (state Λ)) (ε2 : cfg Λ -> nonnegreal),
          ⌜reducible (e1, σ1)⌝ ∗
          ⌜ exists r, forall ρ, (ε2 ρ <= r)%R ⌝ ∗
          ⌜erasable μ σ1 ⌝ ∗
          ⌜((SeriesC (λ ρ : expr Λ * state Λ, ((μ ≫= λ σ2 : state Λ, prim_step e1 σ2) ρ * ε2 ρ)%R)) = ε)%R ⌝ ∗
          ⌜pgl (σ2 ← μ; prim_step e1 σ2) R nnreal_zero⌝ ∗
          ∀ e2 σ2, ⌜ R (e2, σ2)⌝ ={∅}=∗ exec_stutter (fun ε' => Z (e2, σ2) ε') (ε2 (e2, σ2)))
    ⊢ glm e1 σ1 ε Z.
  Proof.
    iIntros "(% & % & % & % & % & % & %Hε & % & H)".
    rewrite {1}glm_unfold.
    iRight. iLeft.
    iExists _,μ,nnreal_zero,_.
    iSplit; [done|].
    iSplit; [done|].
    iSplit; [done|].
    iSplit. { iPureIntro. simpl. rewrite Hε. lra. }
    iSplit; done.
  Qed.
