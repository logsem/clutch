(* Pointwise differential privacy of exact_cache.  Ported from
   [clutch.diffpriv.examples.pointwise_eq_exact_cache] to the GENERIC language.

   A mechanism M may only satisfy pointwise DP. We would like to prove that
   (exact_cache M) is also differentially private, and that caching provides the
   expected cost reduction in privacy budget.

   The whole development below was left COMMENTED OUT in the original (it
   contained admits and open TODOs).  The port therefore only retargets the
   imports and pins the (sampling-free) signature: this development performs no
   sampling, so an empty distribution signature suffices. *)
