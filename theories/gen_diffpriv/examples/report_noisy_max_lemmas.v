(** Supporting lemmas for the generic report-noisy-max port (Laplace noise).
    Ported from [diffpriv.examples.report_noisy_max_lemmas]: the math/list lemmas
    and the [laplace_map] draw couplings are language-independent; the
    state-level presampling lemmas are reformulated over the generic one-[stapes]
    map of [gen_prob_lang] (the [prob_lang]-specific [tapes_laplace]/[Tape_Laplace]/
    [state_step_laplace] are replaced by the generic [stapes]/[state_step]). *)
...
Section rnm_lemmas.
  ...
  (** Laplace-specific unfold of the generic [state_step]: a Laplace tape steps
      by a [laplace_rat] draw (appending its injection to the tape).  This
      reduces the generic [match sig_sample …] (via [sig_sample_at]) and composes
      the two [dmap]s — the gen analogue of [prob_lang]'s [state_step_laplace_unfold]. *)
  Lemma state_step_laplace_unfold σ α num den mean zs :
    stapes σ !! α = Some (LapT num den mean zs) →
    state_step S σ α =
      dmap (λ z : Z, state_upd_stapes (<[α := LapT num den mean (zs ++ [z])]>) σ)
        (laplace_rat num den mean).
  Proof.
    intros Hlook.
    erewrite state_step_unfold; last exact Hlook.
    rewrite (sig_sample_at laplace_family S (num, den, mean)).
    rewrite dmap_comp.
...
Fixpoint laplace_presample_list σ ls:=
  match ls with
  | [] => dret σ
  | hd :: tl => dbind (λ σ', laplace_presample_list σ' tl) (state_step S σ hd)
  end.
...
Fixpoint replace_laplace_tape num den σ ls :=
  ...
  | hd::tl =>
      let '(ι, mean, ls, z) :=hd in
        state_upd_stapes <[ι := LapT num den mean (ls++[z])]> (replace_laplace_tape num den σ tl)
  end.

Lemma laplace_presample_list_rewrite_notin l tl x σ num den :
  l ∉ tl.*1.*1.*1 ->
  replace_laplace_tape num den (state_upd_stapes <[l:=x]> σ)
    tl =
...
Lemma laplace_presample_list_rewrite num den l σ (Hproof: (0 < IZR num / IZR (2*den))%R):
  Forall (λ '(ι, loc, lis), stapes σ!!ι = Some (LapT num (2*den) loc lis)) l ->
  NoDup (l.*1.*1) ->
...
Lemma replace_laplace_tape_zip ls zs num den σ:
  ...
  Forall
    (λ '(ι, loc, lis, z),
       stapes (replace_laplace_tape num (2 * den) σ (zip ls zs)) !! ι =
       Some (LapT num (2 * den) loc (lis ++ [z])))
    (zip ls zs).
Proof.
  revert zs σ.
  induction ls as [|[[]]? IHl].
...
Lemma laplace_state_list_coupl num den ls ls' σ σ':
  ...
  (NoDup ls.*1.*1) ->
  (NoDup ls'.*1.*1) ->
  Forall (λ '(ι, loc, lis), stapes σ!!ι = Some (LapT num (2*den) loc lis)) ls ->
  Forall (λ '(ι, loc, lis), stapes σ'!!ι = Some (LapT num (2*den) loc lis)) ls' ->
  DPcoupl (laplace_presample_list σ ls.*1.*1)
    ...
    (λ σf σf',
       ∃ zs zs', length zs = length zs' /\ (length zs = length ls)%nat /\
                 ...
                 σf' = (replace_laplace_tape num (2 * den) σ' (zip ls' zs'))(*  /\  *)
                 (* Forall (λ '(ι, loc, lis, z), stapes σf!!ι = Some (LapT num (2*den) loc (lis ++ [z]))) (zip ls zs) /\ *)
                 (* Forall (λ '(ι, loc, lis, z), stapes σf'!!ι = Some (LapT num (2*den) loc (lis ++ [z]))) (zip ls' zs') *)
    ) (IZR num / IZR den) 0.
Proof.
...
Lemma laplace_presample_list_erasable num den σ ls (Hineq:(0 < IZR num / IZR (2 * den))%R):
  NoDup ls.*1.*1->
  Forall (λ '(ι, loc, lis), stapes σ!!ι = Some (LapT num (2*den) loc lis)) ls ->
  erasable (Λ := gen_lang S) (laplace_presample_list σ ls.*1.*1) σ.
Proof.
  revert σ.
  ...
  - intros ? Hpos. apply IHls.
    ...
    destruct!/=.
    rewrite (state_step_laplace_unfold _ _ _ _ _ _ H0) in Hpos.
    simpl in *.
    inv_distr; last lra.
    simpl.
    clear -H1 Hnodup.
...
Lemma replace_laplace_tape_heap num den h l ls:
...
Section coupling_rule.
  ...
  Lemma hoare_couple_laplace_list_update xιs xιs' zs zs' ls ls' σ σ' num den:
    ...
    ==∗
    (stapes_auth 1 (stapes (replace_laplace_tape num (2 * den) σ (zip ls zs)))) ∗
     spec_tapes_auth (stapes (replace_laplace_tape num (2 * den) σ' (zip ls' zs'))) ∗
  ...
  Proof.
    ...
    set (ls' := zip (zip xιs'.*2 xιs'.*1) (replicate N ([]:list Z))).
    iAssert (⌜Forall (λ '(ι, loc, lis), stapes σ!!ι = Some (LapT num (2*den) loc lis)) ls⌝)%I as "%".
    ...
    - simpl.
      ...
      destruct σ, σ'.
      rewrite !replace_laplace_tape_heap.
      iFrame. 
