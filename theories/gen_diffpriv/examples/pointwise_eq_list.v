(* Examples of how mapping pointwise-equal functions (f,f') over a list yields
   pointwise-equal results.  Ported from
   [clutch.diffpriv.examples.pointwise_eq_list] to the GENERIC language.

   This case study uses no sampling at all (the queries are opaque [val]s), so
   the port is a mechanical retargeting of imports: the only [gen]-specific
   adjustment is pinning the spec-context [fill] to the development's (here
   sampling-free) signature.  The [map] import of the original is dropped — only
   [list]'s [list_map]/[list_cons] are used. *)
...
Section pweq_list.
  ...
  Proof.
    ...
    - rewrite {2}/list_map.
      ...
      iSpecialize ("IH" $! lv' hl').
      iMod ("IH" with "rhs") as "(% & rhs)". tp_normalise.
      rewrite /list_cons ; tp_pures.
      tp_bind (f' _). iMod (f'_safe with "rhs") as "(% & rhs)". tp_normalise. tp_pures. iFrame. done.
  Qed.
  ...
  Proof using ff_pweq f_safe f'_safe.
    ...
    - destruct HRES as [r0 ->].
      ...
      iIntros "%v (%v' & rhs & %hvv')". tp_normalise. tp_pures. wp_pures.
    ...
    - iApply (wp_strong_mono'' $! (f_safe _)). iIntros.
      iMod (f'_safe with "rhs") as "(%v' & rhs)". tp_normalise. tp_pures. wp_pures.
  ...
  Proof using ff_pweq f_safe f'_safe.
    ...
    - tp_bind (f' _). wp_bind (f _).
      ...
      iSpecialize ("h0" with "rhs").
      iApply (wp_strong_mono'' with "h0").
      iIntros "%v0 (%v0' & rhs & %hvv0')". tp_normalise. tp_pures. wp_pures.

      tp_bind (f' _). wp_bind (f _).
      iPoseProof (ff_pweq _ #1 r1) as "h1".
      iSpecialize ("h1" with "rhs").
      iApply (wp_strong_mono'' with "h1").
      iIntros "%v1 (%v1' & rhs & %hvv1')". tp_normalise. tp_pures. wp_pures.
    ...
    - tp_bind (f' _). wp_bind (f _).
      iApply (wp_strong_mono'' $! (f_safe _)). iIntros.
      iMod (f'_safe with "rhs") as "(% & rhs)". tp_normalise. tp_pures. wp_pures.
      tp_bind (f' _). wp_bind (f _).
      iApply (wp_strong_mono'' $! (f_safe _)). iIntros.
      iMod (f'_safe with "rhs") as "(% & rhs)". tp_normalise. tp_pures. wp_pures.
      iModIntro. iFrame. iPureIntro. intros.
  ...
  Proof using ff_pweq f_safe f'_safe.
    ...
    - destruct HRES as (LRES & hres & res_qs).
      ...
      + destruct hqs as (qs' & hqs & hqs').
        ...
        tp_normalise ; tp_pures ; wp_pures. tp_bind (f' _) ; wp_bind (f _).
        iPoseProof (ff_pweq _ Q RQ) as "hQ".
        iSpecialize ("hQ" with "rhs").
        iApply (wp_strong_mono'' with "hQ").
        iIntros "%vQ (%vQ' & rhs & %hQQ')". tp_normalise.
        rewrite /list_cons ; tp_pures ; wp_pures ; rewrite -/list_cons.
    ...
    -
      ...
      + destruct hqs as (qs' & hqs & hqs').
        ...
        *
          ...
          tp_bind (list_map _ _). iMod ((hh' _ _) with "rhs") as "(%v' & rhs)". 1: done. tp_normalise. tp_pures. tp_bind (f' _).
          iMod (f'_safe _ _ with "rhs") as "(% & rhs)". tp_normalise. rewrite {1}/list_cons. tp_pures.
          wp_bind (list_map _ _).
