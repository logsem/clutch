(* A simple heap-backed map ADT (associative linked list).  Ported from
   [clutch.diffpriv.examples.map] to the GENERIC language.

   This development performs no sampling (it only uses the heap spec ops
   refs/load/store), so an empty distribution signature suffices.  The only
   [gen]-specific adjustments are: retargeting the imports, pinning the
   spec-context [fill] to the (sampling-free) signature, and interleaving
   [tp_normalise] after the spec heap tactics ([tp_load]/[tp_store]) — which in
   [gen] leave the spec hypothesis FOCUSED at the just-touched subterm, so a
   following [tp_pures] cannot see the surrounding redex unless we re-fold the
   spec into [fill K (full-expr)]. *)
...
(* Simple map as an associative linked list, based on the examples
  ...
  Proof.
    iIntros "H Hr".
    rewrite /find_list. iInduction vs as [|(k', v') vs] "IH" forall (l).
    - tp_pures. rewrite /assoc_list. rewrite /rec_unfold. tp_pures. tp_load. tp_normalise. tp_pures. iModIntro. iFrame.
    - tp_pures. iDestruct "H" as (?) "(Hl&Hassoc)".
      rewrite /rec_unfold. tp_pure. tp_load. tp_normalise. tp_pures. case_bool_decide as Hcase.
  ...
  Proof.
    iIntros "H Hr".
    rewrite /find_list. iInduction vs as [|(k', v') vs] "IH" forall (l).
    - tp_pures. rewrite /assoc_list. rewrite /rec_unfold. tp_pure. tp_load. tp_normalise. tp_pures. iModIntro. iFrame.
      rewrite /=. iFrame. destruct (bool_decide _) => //=.
    - tp_pures. iDestruct "H" as (?) "(Hl&Hassoc)".
      rewrite /rec_unfold. tp_pure. tp_load. tp_normalise. tp_pures.
      destruct (bool_decide (#k' = #z)) eqn:Hbool.
      { apply bool_decide_eq_true_1 in Hbool.
  ...
  Proof.
    ...
    tp_pures.
    iDestruct "Hm" as (ll vs) "(Hll&%Heq&Hassoc)".
    tp_load. tp_normalise. tp_pures.
    tp_bind (cons_list _ _).
    iMod (spec_cons_list with "[$] [$]") as (?)"(Hr&Halloc)".
    tp_normalise. tp_store. iModIntro.
