...
Class diffprivRGS (Sg : Sig) Σ := DiffprivRGS {
                        diffprivRGS_diffprivGS :: diffprivGS Sg Σ;
...
Definition na_ownP `{!diffprivRGS Sg Σ} := na_own diffprivRGS_nais.
Definition na_invP `{!diffprivRGS Sg Σ} := na_inv diffprivRGS_nais.
Definition na_closeP `{!diffprivRGS Sg Σ} P N E := (▷ P ∗ na_ownP (E ∖ ↑N) ={⊤}=∗ na_ownP E)%I.
...
Section semtypes.
  ...
  Definition refines_def (E : coPset) (e : expr) (e' : expr) (A : lrel Σ)
    : iProp Σ :=
    (∀ (K : list (@ectxi_language.ectx_item (gen_ectxi_lang Sg))) ε, ⤇ fill K e' -∗
          na_ownP E -∗
          ↯ ε -∗
          ⌜ (0 < ε)%R ⌝ -∗
  ...
  Definition lrel_tape : lrel Σ := LRel (λ w1 w2,
    ∃ (α1 α2 : loc) (i : nat) (pv : val), ⌜w1 = #lbl:α1⌝ ∧ ⌜w2 = #lbl:α2⌝ ∧
      inv (logN .@ (α1, α2)) (α1 ↪ (i, pv, []) ∗ α2 ↪ₛ (i, pv, [])))%I.
...
Section semtypes_properties.
  ...
  Proof.
    iIntros (?) "[Hl1 Hl2] /=".
    iDestruct "Hl1" as (l' l1' i1 pv1) "[% [% #Hl1]]". simplify_eq.
    iDestruct "Hl2" as (l l2' i2 pv2) "[% [% #Hl2]]". simplify_eq.
    destruct (decide (l1' = l2')) as [->|?]; eauto.
    iInv (logN.@(l, l1')) as "[>Hl ?]" "Hcl".
    iInv (logN.@(l, l2')) as "[>Hl' ?]" "Hcl'".
    simpl. iExFalso.
  ...
  Proof.
    iIntros (?) "[Hl1 Hl2] /=".
    iDestruct "Hl1" as (l1' l' i1 pv1) "[% [% #Hl1]]". simplify_eq.
    iDestruct "Hl2" as (l2' l i2 pv2) "[% [% #Hl2]]". simplify_eq.
    destruct (decide (l1' = l2')) as [->|?]; eauto.
    iInv (logN.@(l1', l)) as "(? & >Hl)" "Hcl".
    iInv (logN.@(l2', l)) as "(? & >Hl')" "Hcl'".
    simpl. iExFalso.
...
Section related_facts.
  ...
  Proof.
    ...
    iIntros "H". iIntros (j ε) "Hr Hna Herr /=".
    iMod "H" as "H". iApply ("H" with "Hr Hna Herr").
  Qed.
  ...
  Proof.
    ...
    rewrite refines_eq /refines_def.
    iIntros (K ε) "Hs own_FN Herr".
    iDestruct ("Hclose" with "[$HP $own_FN]") as "own_F".
    iMod "own_F". iApply ("IH" with "Hs own_F Herr").
  Qed.
...
Section monadic.
  ...
  Lemma refines_bind (K K' : list (@ectxi_language.ectx_item (gen_ectxi_lang Sg))) E A A' e e' :
    (REL e << e' @ E : A) -∗
    (∀ v v', A v v' -∗
  ...
  Proof.
    ...
    rewrite -fill_app.
    iSpecialize ("Hm" $! (K' ++ K'') with "[$Hj] Hnais Herr Hpos").
    iApply wp_bind.
    iApply (wp_wand with "Hm").
    iIntros (v). iDestruct 1 as (v' ε') "[Hj (Hnais & Herr & Hpos & HA)]".
    rewrite fill_app.
    iApply ("Hf" with "HA Hj Hnais Herr Hpos").
  Qed.

  Lemma refines_ret_na E e1 e2 v1 v2 (A : lrel Σ) :
    @IntoVal (gen_lang Sg) e1 v1 →
    @IntoVal (gen_lang Sg) e2 v2 →
    (na_ownP E ={⊤}=∗ na_ownP ⊤ ∗ A v1 v2) -∗
    REL e1 << e2 @ E : A.
  Proof.
    rewrite /IntoVal.
    rewrite refines_eq /refines_def.
    iIntros (<-<-) "HFA". iIntros (K ε) "HK Hnais Herr Hpos /=".
    iMod ("HFA" with "Hnais") as "[HF HA]".
    iApply (wp_value' (Λ := gen_lang Sg)). iExists _, _. iFrame.
  Qed.
  ...
  Proof.
    rewrite /IntoVal.
    rewrite refines_eq /refines_def.
    iIntros (<-<-) "HFA". iIntros (j ε) "Hj Hnais Herr Hpos /=".
    iMod "HFA" as "[HF HA]".
    iApply (wp_value' (Λ := gen_lang Sg)). iExists _. iFrame.
