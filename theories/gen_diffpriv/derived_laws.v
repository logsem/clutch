(** Derived array laws for the generic DP weakest precondition.  Ported from
    [clutch.diffpriv.derived_laws]: the [array] connective ([↦∗]) and the
    offset load/store/alloc rules, used by the proof-mode heap tactic instance
    and by array-manipulating case studies.  The lemmas are all Λ-generic — only
    the [diffprivGS Sg]/canonical-structure setup is signature-specific. *)
...
Definition array `{!diffprivGS Sg Σ} (l : loc) (dq : dfrac) (vs : list val) : iProp Σ :=
...
Section lifting.
  ...
  Lemma wp_load_offset_vec E l dq sz (off : fin sz) (vs : vec val sz) :
    {{{ ▷ l ↦∗{dq} vs }}} ! #(l +ₗ off) @ E {{{ RET vs !!! off; l ↦∗{dq} vs }}}.
