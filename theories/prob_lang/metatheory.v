...
Next Obligation.
  ...
  { rewrite 2?SeriesC_0; [lra| |].
    - intros []. case_bool_decide; [|done]. rewrite bool_decide_eq_false_2 //.
      by intros (?&?&?).
    - intros []. case_bool_decide; [|done]. rewrite bool_decide_eq_false_2 //.
      by intros (?&?&?). }
  destruct (decide (α' ∈ dom σ1'.(tapes))); last first.
  { rewrite 2?SeriesC_0; [lra| |].
    - intros []. case_bool_decide; [|done]. rewrite bool_decide_eq_false_2 //. by intros (?&?&?).
    - intros []. case_bool_decide; [|done]. rewrite bool_decide_eq_false_2 //. by intros (?&?&?). }
