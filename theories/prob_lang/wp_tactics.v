...
(*
(* A generic WP *)
Class Gwp (PROP EXPR VAL A : Type) := {
    gwp : A → coPset → EXPR → (VAL → PROP) → PROP;
    gwp_default : A
  }.
Global Arguments gwp {_ _ _ _ _} _ _ _%E _%I.
Global Instance: Params (@gwp) 9 := {}.
Global Arguments gwp_default : simpl never.

(** Notations for generic weakest preconditions *)
(** Notations without binder -- only parsing because they overlap with the
notations with binder. *)
Local Notation "'WP' e @ s ; E {{ Φ } }" := (gwp s E e%E Φ)
  (at level 20, e, Φ at level 200, only parsing) : bi_scope.
Local Notation "'WP' e @ E {{ Φ } }" := (gwp gwp_default E e%E Φ)
  (at level 20, e, Φ at level 200, only parsing) : bi_scope.
Local Notation "'WP' e {{ Φ } }" := (gwp gwp_default ⊤ e%E Φ)
  (at level 20, e, Φ at level 200, only parsing) : bi_scope.
...
(** A basic set of requirements for a weakest precondition *)
Class TwpTacticsBase (Σ : gFunctors) (A : Type) `{Twp (iProp Σ) expr val A} `{!invGS_gen hlc Σ}  := {
  twptac_twp_value E Φ v a : Φ v ⊢ WP (of_val v) @ a; E [{ Φ }];
  twptac_twp_fupd E Φ e a : WP e @ a; E [{ v, |={E}=> Φ v }] ⊢ WP e @ a; E [{ Φ }];
  twptac_twp_bind K `{!LanguageCtx K} E e Φ a :
    WP e @ a; E [{ v, WP K (of_val v) @ a; E [{ Φ }] }] ⊢ WP (K e) @ a; E [{ Φ }];
}.

(** Pure evaluation steps with or without laters  *)
Class TwpTacticsPure Σ A `{Twp (iProp Σ) expr val A} := {
  twptac_twp_pure_step E e1 e2 φ n Φ a :
    PureExec φ n e1 e2 →
    φ →
...
(** Heap *)
Class TwpTacticsHeap Σ A `{Twp (iProp Σ) expr val A} := {
  twptac_mapsto : loc → dfrac → val → iProp Σ;
  twptac_mapsto_array : loc → dfrac → (list val) → iProp Σ;

  twptac_twp_alloc E v a :
    [[{ True }]]
      (Alloc (Val v)) @ a; E
    [[{ l, RET LitV (LitLoc l); twptac_mapsto l (DfracOwn 1) v }]];
 twptac_twp_allocN E v n a :
    (0 < n)%Z →
    [[{ True }]]
      (AllocN (Val $ LitV $ LitInt $ n) (Val v)) @ a; E
    [[{ l, RET LitV (LitLoc l); twptac_mapsto_array l (DfracOwn 1) (replicate (Z.to_nat n) v) }]];
  twptac_twp_load E v l dq a :
    [[{ twptac_mapsto l dq v }]]
      Load (Val $ LitV $ LitLoc l) @ a; E
    [[{ RET v; twptac_mapsto l dq v }]];
  twptac_twp_store E v v' l a :
                                        [[{ twptac_mapsto l (DfracOwn 1) v' }]]
                                          Store (Val $ LitV (LitLoc l)) (Val v) @ a; E
                                                                                       [[{ RET LitV LitUnit; twptac_mapsto l (DfracOwn 1) v }]]
  }.

Section twp_tactics.
  Context `{TwpTacticsBase Σ A}.

  Lemma tac_twp_expr_eval Δ a E Φ e e' :
    (∀ (e'':=e'), e = e'') →
    envs_entails Δ (WP e' @ a; E [{ Φ }]) → envs_entails Δ (WP e @ a; E [{ Φ }]).
  Proof. by intros ->. Qed.

  Lemma tac_twp_pure_later `{!TwpTacticsPure Σ A} Δ E K e1 e2 φ n Φ a :
    PureExec φ n e1 e2 →
    φ →
    envs_entails Δ (WP (fill K e2) @ a; E [{ Φ }]) →
    envs_entails Δ (WP (fill K e1) @ a; E [{ Φ }]).
  Proof.
    ...
    (* We want [pure_exec_fill] to be available to TC search locally. *)
    pose proof @pure_exec_fill.
    rewrite HΔ' -twptac_twp_pure_step //.
  Qed.

  Lemma tac_twp_value_nofupd Δ E Φ v a :
    envs_entails Δ (Φ v) → envs_entails Δ (WP (of_val v) @ a; E [{ Φ }]).
  Proof. rewrite envs_entails_unseal=> ->. apply twptac_twp_value. Qed.

  Lemma tac_twp_value' Δ E Φ v a :
    envs_entails Δ (|={E}=> Φ v) → envs_entails Δ (WP (of_val v) @ a; E [{ Φ }]).
  Proof. rewrite envs_entails_unseal=> ->. by rewrite -twptac_twp_fupd -twptac_twp_value. Qed.

  Lemma tac_twp_bind K Δ E Φ e f a :
    f = (λ e, fill K e) → (* as an eta expanded hypothesis so that we can `simpl` it *)
    envs_entails Δ (WP e @ a; E [{ v, WP f (Val v) @ a; E [{ Φ }] }])%I →
    envs_entails Δ (WP fill K e @ a; E [{ Φ }]).
  Proof. rewrite envs_entails_unseal=> -> ->. by apply: twptac_twp_bind. Qed.
...
Tactic Notation "wp_pure" open_constr(efoc) :=
  ...
  | |- envs_entails _ (twp ?s ?E ?e ?Q) =>
    ...
    reshape_expr e ltac:(fun K e' =>
      unify e' efoc;
      eapply (tac_twp_pure_later _ _ K e');
      [iSolveTC                       (* PureExec *)
      |try solve_vals_compare_safe    (* The pure condition for PureExec -- handles trivial goals *)
      |wp_finish                      (* new goal *)
      ])
    || fail "wp_pure: cannot find" efoc "in" e "or" efoc "is not a redex"
...
Ltac wp_apply_core lem tac_suc tac_fail := first
  [iPoseProofCore lem as false (fun H =>
     ...
     | |- envs_entails _ (wp ?s ?E ?e ?Q) =>
         reshape_expr e ltac:(fun K e' => wp_bind_core K; tac_suc H)
     | |- envs_entails _ (twp ?s ?E ?e ?Q) =>
       reshape_expr e ltac:(fun K e' => twp_bind_core K; tac_suc H)
     | _ => fail 1 "wp_apply: not a 'wp'"
     end)
  |tac_fail ltac:(fun _ => wp_apply_core lem tac_suc tac_fail)
  |let P := type of lem in
...
Section heap_tactics.
  ...
  Context `{TwpTacticsBase Σ A, !TwpTacticsHeap Σ A}.
  ...
  Lemma tac_twp_alloc Δ E j K v Φ a :
    (∀ (l : loc),
        match envs_app false (Esnoc Enil j (twptac_mapsto l (DfracOwn 1) v)) Δ with
        | Some Δ'' =>
            envs_entails Δ'' (WP fill K (Val $ LitV $ LitLoc l) @ a ; E [{ Φ }])
        | None => False
        end) →
  ...
  Proof.
    rewrite envs_entails_unseal=> HΔ.
    rewrite -twptac_twp_bind.
    eapply bi.wand_apply; first exact: twptac_twp_alloc.
    rewrite left_id. apply bi.forall_intro=> l.
    specialize (HΔ l).
    destruct (envs_app _ _ _) as [Δ''|] eqn:HΔ'; [ | contradiction ].
    rewrite envs_app_sound //; simpl.
    apply bi.wand_intro_l.
  ...
  Lemma tac_twp_allocN Δ E j K v n Φ a :
    (0 < n)%Z →
    (∀ l,
        match envs_app false (Esnoc Enil j (twptac_mapsto_array l (DfracOwn 1) (replicate (Z.to_nat n) v))) Δ with
        | Some Δ'' =>
            envs_entails Δ'' (WP fill K (Val $ LitV $ LitLoc l) @ a; E [{ Φ }])
        | None => False
        end) →
  ...
  Proof.
    rewrite envs_entails_unseal=> ? HΔ.
    rewrite -twptac_twp_bind.
    eapply bi.wand_apply; first exact: twptac_twp_allocN.
    rewrite left_id.
    apply bi.forall_intro=> l.
    specialize (HΔ l).
    destruct (envs_app _ _ _) as [Δ''|] eqn:HΔ'; [ | contradiction ].
    rewrite envs_app_sound //; simpl.
    apply bi.wand_intro_l.
  ...
  Lemma tac_twp_load Δ E i K b l dq v Φ a :
    envs_lookup i Δ = Some (b, twptac_mapsto l dq v)%I →
    envs_entails Δ (WP fill K (Val v) @ a; E [{ Φ }]) →
    envs_entails Δ (WP fill K (Load (Val $ LitV $ LitLoc l)) @ a; E [{ Φ }]).
  Proof.
    rewrite envs_entails_unseal=> ? Hi.
    rewrite -twptac_twp_bind. eapply bi.wand_apply; [exact: twptac_twp_load|].
    rewrite envs_lookup_split //; simpl.
    destruct b; simpl.
    - iIntros "[#$ He]". iIntros "_". iApply Hi. iApply "He". iFrame "#".
  ...
  Lemma tac_twp_store Δ E i K l v v' Φ a :
    envs_lookup i Δ = Some (false, twptac_mapsto l (DfracOwn 1) v)%I →
    match envs_simple_replace i false (Esnoc Enil i (twptac_mapsto l (DfracOwn 1) v')) Δ with
    | Some Δ'' => envs_entails Δ'' (WP fill K (Val $ LitV LitUnit) @ a; E [{ Φ }])
    | None => False
    end →
    envs_entails Δ (WP fill K (Store (Val $ LitV $ LitLoc l) (Val v')) @ a; E [{ Φ }]).
  Proof.
    rewrite envs_entails_unseal=> ? Hcnt.
    destruct (envs_simple_replace _ _ _) as [Δ''|] eqn:HΔ''; [ | contradiction ].
    rewrite -twptac_twp_bind. eapply bi.wand_apply; [by eapply twptac_twp_store|].
    rewrite envs_simple_replace_sound //; simpl.
    rewrite right_id. by apply bi.sep_mono_r, bi.wand_mono.
  Qed.
