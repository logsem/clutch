...
From self.common Require Import language ectx_language exec.
From self.prob_lang Require Import lang notation metatheory.
From self.prob_lang.typing Require Import types contextual_refinement.
...
Lemma lim_exec_val_of_val_not_b_zero e b σ :
  is_Some (to_val e) → e ≠ of_val #b → lim_exec_val ((e = #b)%E, σ) #true = 0.
Proof.
  intros H H0.
  rewrite lim_exec_val_rw.
...
Lemma lim_exec_val_is_b_test e σ (b : bool) :
  lim_exec_val (e, σ) #b = lim_exec_val ((e = #b)%E, σ) #true.
Proof.
...
Lemma loop_zero_mass n σ x :
  exec_val n (loop, σ) x = 0.
Proof.
...
Lemma lim_exec_val_of_val_not_true_zero (e : expr) σ :
  (∃ v, e = of_val v) →
  e ≠ #true →
  lim_exec_val ((if: e then #() else loop)%E, σ) #() = 0.
Proof.
  intros H H0.
  rewrite lim_exec_val_rw.
  erewrite <-sup_seq_const. do 2 f_equal.
