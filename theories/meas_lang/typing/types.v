...
Definition rec_unfold := RecVC binders.BAnon (binders.BNamed "x") (VarC (binders.BNamed "x")).
Definition unpack := RecVC binders.BAnon (binders.BNamed "x")
                     (RecC binders.BAnon (binders.BNamed "y")
                     (AppC (VarC (binders.BNamed "y")) (VarC (binders.BNamed "x")))).

Notation "'unpack:' x := e1 'in' e2" := (unpack e1%E (Lam x e2%E))
  (at level 200, x at level 1, e1, e2 at level 200, only parsing) : expr_scope.

Notation "'unpack:' x := e1 'in' e2" := (unpack e1%E (LamV x e2%E))
...
Proof. destruct op; simpl; rewrite /bin_op_eval; repeat case_match; try done.
       (** @Markus, definition of bin_op_eval incorrect. 
           We probably need a definition of val_compare_safe
       *)
Admitted.
...
Proof. destruct op; rewrite /bin_op_eval; repeat case_match; try done.
       (** same as before *)
Admitted. (* destruct op; naive_solver. Qed. *)
