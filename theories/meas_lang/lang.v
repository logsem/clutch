...
Section expr_measurability.
  ...
  *)
  ...
  Qed.
    ...
    (* The set of vals such that
      - Are of the form LitVC v
      - Have v drawn from a measurable set base_lits generated by B

      No need to decompose in this proof

     *)
    (*
    rewrite /vcov_lit.
    rewrite <- (base_lit_shape_decomp S).
    rewrite preimage_bigcup.
    rewrite setI_bigcupr.
    apply bigcup_measurable.
    move=> k _.
    rewrite /preimage/setI/=.
    eexists (LitV (gen_base_lit (base_lit_shape_enum k))).
    { simpl. by apply gen_base_lit_generator. }
     *)
