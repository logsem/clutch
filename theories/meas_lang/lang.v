...
Section expr_measurability.
  ...
  *)
  ...
  Proof.
    rewrite /ecov_fst.
    (* FIXME: This changes each proof, make it simple *)
    have X : [set e | ∃ x : expr_pre, e = FstC x] = \bigcup_n [set FstC x | x in (expr_seq n)].
    { rewrite /bigcup/=.
      apply /predeqP =>y /=.
      split.
      - move=> [x ->].
        destruct (expr_shape_enum_surj (shape_expr x)) as [i Hi].
        exists i; [done|].
        exists x; [|done].
        by rewrite /expr_seq/=.
      - move=> [? _] [x ?] <-.
