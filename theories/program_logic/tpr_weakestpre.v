...
Definition rwp_step {Σ Λ A} `{spec A Σ} `{!irisGS Λ Σ} E s (e1 : expr Λ) (Z : cfg Λ → A → iProp Σ) : iProp Σ :=
  ∀ σ1 a1,
...
Definition rwp_pre {Σ A Λ} `{spec A Σ} `{!irisGS Λ Σ} (s : stuckness)
  ...
  | None => rwp_step E s e1 (λ '(e2, σ2) a2, |={∅,E}=> state_interp σ2 ∗ spec_interp a2 ∗ rwp E e2 Φ)
  end%I.

Lemma rwp_pre_mono {Σ A Λ} `{spec A Σ} `{!irisGS Λ Σ}  s (wp1 wp2 : coPset → expr Λ → (val Λ → iProp Σ) → iProp Σ) :
...
Definition rwp_pre' {Σ A Λ} `{spec A Σ} `{!irisGS Λ Σ} (s : stuckness) :
  (prodO (prodO (leibnizO coPset) (exprO Λ)) (val Λ -d> iProp Σ) → iProp Σ) →
   prodO (prodO (leibnizO coPset) (exprO Λ)) (val Λ -d> iProp Σ) → iProp Σ
  := uncurry3 ∘ rwp_pre s ∘ curry3.

Local Instance exec_coupl_pre_mono {Λ A Σ} `{spec A Σ} `{!irisGS Λ Σ} (s : stuckness) :
...
Qed.
Lemma fupd_rwp' s E e Φ :
  (∀ σ a, state_interp σ ∗ spec_interp a ={E}=∗ state_interp σ ∗ spec_interp a) ∗
...
Proof.
  ...
  { iIntros. iSpecialize ("H" with "[$]"). rewrite rwp_unfold /rwp_pre Heq.
    iMod "H" as "[H1 H2]". by iMod ("Hrwp" with "[$]") as "$". }
  iIntros (σ1 a) "HS".
...
Lemma rwp_fupd' s E e Φ :
  RWP e @ s; E ⟨⟨ v, ∀ σ a, state_interp σ ∗ spec_interp a ={E}=∗
...
Lemma rwp_atomic E1 E2 e s Φ `{!Atomic WeaklyAtomic e} :
