...
From tachis.tachis Require Export expected_time_credits.
...
Class tachisWpGS (Λ : language) (Σ : gFunctors) := TachisWpGS {
  tachisWpGS_invGS :: invGS_gen HasNoLc Σ;
  tachisWpGS_etcGS :: etcGS Σ;

  state_interp : state Λ → iProp Σ;
  costfun :: Costfun Λ;
}.
Global Opaque tachisWpGS_invGS.
Global Opaque tachisWpGS_etcGS.
Global Arguments TachisWpGS {Λ Σ}.

(** * The expected runtime modality [ERM]  *)
Section ERM.
  Context `{!tachisWpGS Λ Σ}.
...
Definition ert_wp_pre `{!tachisWpGS Λ Σ}
...
Local Instance wp_pre_contractive `{!tachisWpGS Λ Σ} : Contractive (ert_wp_pre).
Proof.
...
Local Definition ert_wp_def `{!tachisWpGS Λ Σ} : Wp (iProp Σ) (expr Λ) (val Λ) () :=
...
Local Lemma ert_wp_unseal `{!tachisWpGS Λ Σ} : wp = (@ert_wp_def Λ Σ _).(wp).
