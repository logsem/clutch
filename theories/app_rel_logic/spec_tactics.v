...
Lemma tac_tp_pure `{app_clutchGS Σ} e K' e1 k e2 Δ1 E1 i1 e' ϕ ψ Q n :
...
Lemma tac_tp_store `{app_clutchGS Σ} k Δ1 Δ2 E1 i1 i2 K' e (l : loc) e' e2 v' v Q :
  ...
  envs_lookup_delete false i1 Δ1 = Some (false, ⤇ fill k e, Δ2)%I →
  e = fill K' (Store (# l) e') →
  IntoVal e' v →
  ...
  match envs_simple_replace i2 false
     ( (Esnoc Enil i1 (spec_update E1 (⤇ fill k e2∗ l ↦ₛ v))) ) Δ2 with
  | None => False
...
Lemma tac_tp_load `{app_clutchGS Σ} k Δ1 Δ2 E1 i1 i2 K' e e2 (l : loc) v Q q :
  (∀ P, ElimModal True false false (|={E1}=> P) P Q Q) →
  envs_lookup_delete false i1 Δ1 = Some (false, ⤇ fill k e, Δ2)%I →
  e = fill K' (Load #l) →
  ...
  e2 = fill K' (of_val v) →
  match envs_simple_replace i2 false
    (Esnoc Enil i1 (spec_update E1 (⤇ fill k e2∗(l ↦ₛ{q} v))%I) ) Δ2 with
  | Some Δ3 => envs_entails Δ3 Q
  | None    => False
  end →
  envs_entails Δ1 Q.
...
Lemma tac_tp_alloc `{app_clutchGS Σ} k Δ1 E1 i1 K' e e' v Q :
  (∀ P, ElimModal True false false (|={E1}=> P) P Q Q) →
  nclose specN ⊆ E1 →
...
Lemma tac_tp_alloctape `{app_clutchGS Σ} k Δ1 E1 i1 K' e N z Q :
  (∀ P, ElimModal True false false (|={E1}=> P) P Q Q) →
  TCEq N (Z.to_nat z) →
  nclose specN ⊆ E1 →
  envs_lookup i1 Δ1 = Some (false, refines_right k e)%I →
...
Lemma tac_tp_rand `{app_clutchGS Σ} k Δ1 Δ2 E1 i1 i2 K' e e2 (l : loc) N z n ns Q :
  (∀ P, ElimModal True false false (|={E1}=> P) P Q Q) →
  TCEq N (Z.to_nat z) →
  nclose specN ⊆ E1 →
