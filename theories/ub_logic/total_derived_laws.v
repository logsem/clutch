...
From clutch.ub_logic Require Export total_primitive_laws.
...
Definition array `{!ub_clutchGS Σ} (l : loc) (dq : dfrac) (vs : list val) : iProp Σ :=
...
Lemma twp_allocN E v n s :
  ...
  Proof.
    iIntros (? Φ) "_ HΦ".
    iApply twp_allocN_seq; auto; try lia.
    iIntros (l) "Hlm".
    iApply "HΦ".
  ...
  Proof.
    iIntros (? Φ) "_ HΦ".
    iApply (twp_allocN with "[//] [HΦ]"); try lia.
    iIntros (l) "Hl".
...
Lemma twp_load_offset_vec E l dq sz (off : fin sz) (vs : vec val sz) s :
  [[{ l ↦∗{dq} vs }]] ! #(l +ₗ off) @ s; E [[{ RET vs !!! off; l ↦∗{dq} vs }]].
  Proof. apply twp_load_offset. by apply vlookup_lookup. Qed.


Lemma twp_store_offset E l off vs v s :
  is_Some (vs !! off) →
  [[{ l ↦∗ vs }]] #(l +ₗ off) <- v @ s; E [[{ RET #(); l ↦∗ <[off:=v]> vs }]].
Proof.
  iIntros ([w Hlookup] Φ) "Hl HΦ".
  iDestruct (update_array l _ _ _ _ Hlookup with "Hl") as "[Hl1 Hl2]".
  iApply (twp_store with "Hl1").
  iIntros "Hl1".
  iApply "HΦ". iApply "Hl2". iApply "Hl1".
Qed.


Lemma twp_store_offset_vec E l sz (off : fin sz) (vs : vec val sz) v s :
