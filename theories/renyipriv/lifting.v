...
From clutch.renyipriv Require Import weakestpre.
...
Lemma wp_lift_step_prog_couple E Φ e1 s :
  ...
  (∀ σ1 e1' σ1' α1 ρ1,
      state_interp σ1 ∗ spec_interp (e1', σ1') ∗ err_interp α1 ρ1 ={E, ∅}=∗
      prog_coupl e1 σ1 e1' σ1' α1 ρ1 (λ e2 σ2 e2' σ2' α2 ρ2,
        ▷ |={∅, E}=> state_interp σ2 ∗ spec_interp (e2', σ2') ∗
                     err_interp α2 ρ2 ∗ WP e2 @ s; E {{ Φ }}))
  ⊢ WP e1 @ s; E {{ Φ }}.
Proof.
  iIntros (Hv) "H".
  iApply wp_lift_step_couple.
...
Lemma wp_lift_prim_steps_coupl E Φ e1 s :
  to_val e1 = None →
  (∀ σ1 e1' σ1' α ρ,
      state_interp σ1 ∗ spec_interp (e1', σ1') ∗ err_interp α ρ ={E, ∅}=∗
      ∃ (S : cfg Λ → cfg Λ → Prop) (ρ1 ρ2 : nonnegreal),
        ⌜ρ = (ρ1 + ρ2)%NNR⌝ ∗
        ⌜reducible (e1, σ1)⌝ ∗
        ⌜reducible (e1', σ1')⌝ ∗
        ⌜RDPcoupl (prim_step e1 σ1) (prim_step e1' σ1') S α ρ1⌝ ∗
        ∀ e2 σ2 e2' σ2', ⌜S (e2, σ2) (e2', σ2')⌝ ={∅}=∗ ▷ |={∅,E}=>
          state_interp σ2 ∗ spec_interp (e2', σ2') ∗
          err_interp α ρ2 ∗ WP e2 @ s; E {{ Φ }})
  ⊢ WP e1 @ s; E {{ Φ }}.
Proof.
  iIntros (He1) "H".
  iApply wp_lift_step_couple.
  iIntros (σ1 e1' σ1' α ρ) "(Hst & Hsp & Herr)".
  iMod ("H" with "[$Hst $Hsp $Herr]") as
    "[%S [%ρ1 [%ρ2 (%Hρ & %Hred & %Hred' & %Hcpl & H)]]]".
  iModIntro.
  iApply spec_coupl_ret.
  rewrite He1.
  iApply (prog_coupl_steps α ρ1 ρ2 _ S); [done | done | done | done |].
  iIntros (????) "HR".
  iMod ("H" with "HR") as "H".
  iModIntro.
  by iApply spec_coupl_ret.
...
Proof.
  ...
  iIntros (σ1 e1' σ1' α ρ) "(Hst & Hsp & Herr)".
  iMod ("H" with "[$Hst $Hsp $Herr]") as
    "[%S [%ρ1 [%ρ2 (%Hρ & %Hred & %Hcpl & H)]]]".
  iModIntro.
  iApply spec_coupl_ret.
  rewrite He1.
  iApply (prog_coupl_step_l_dret ρ2 ρ1 ρ α) ; [done|done|done|].
  iIntros (??) "HR".
  iMod ("H" with "HR") as "H".
  iModIntro.
  by iApply spec_coupl_ret.
...
Proof.
  ...
  iIntros (σ1 e1' σ1' α ρ) "(Hst & Hsp & Herr)".
  iMod ("H" with "[$Hst $Hsp $Herr]") as
    "[%S [%μ1' [%ρ1 [%ρ2 (%Hρ & %Hred & %Hcpl & %Hera & H)]]]]".
  iModIntro.
  iApply spec_coupl_ret.
  rewrite He1.
  iApply (prog_coupl_step_l_erasable ρ2 ρ1); [done|done|done|done|].
  iIntros (???) "HR".
  iMod ("H" with "HR") as "H".
  iModIntro.
  by iApply spec_coupl_ret.
