...
From clutch.base_logic Require Export renyi_credits.
From clutch.renyipriv Require Export weakestpre ectx_lifting.
...
Class renyiprivGS Σ := HeapG {
  renyiprivGS_invG : invGS_gen HasNoLc Σ;
  (* CMRA for the state *)
  renyiprivGS_heap :: ghost_mapG Σ loc val;
  renyiprivGS_tapes :: ghost_mapG Σ loc tape;
  (* ghost names for the state *)
  renyiprivGS_heap_name : gname;
  renyiprivGS_tapes_name : gname;
  (* CMRA and ghost name for the spec *)
  renyiprivGS_spec :: specG_prob_lang Σ;
  (* CMRA and ghost names for the error *)
  renyiprivGS_error :: rcGS Σ;
}.

Class renyiprivGpreS Σ := RenyiprivGpreS {
  renyiprivGpreS_iris  :: invGpreS Σ;
  renyiprivGpreS_heap  :: ghost_mapG Σ loc val;
  renyiprivGpreS_tapes :: ghost_mapG Σ loc tape;
  renyiprivGpreS_spcec :: specGpreS Σ;
  renyiprivGpreS_err   :: rcGpreS Σ;
}.

Definition renyiprivΣ : gFunctors :=
  #[invΣ;
    ghost_mapΣ loc val;
    ghost_mapΣ loc tape;
    specΣ;
    rcΣ].
Global Instance subG_renyiprivGPreS {Σ} : subG renyiprivΣ Σ → renyiprivGpreS Σ.
Proof. solve_inG. Qed.

Definition heap_auth `{renyiprivGS Σ} :=
  @ghost_map_auth _ _ _ _ _ renyiprivGS_heap renyiprivGS_heap_name.
Definition tapes_auth `{renyiprivGS Σ} :=
  @ghost_map_auth _ _ _ _ _ renyiprivGS_tapes renyiprivGS_tapes_name.
Definition renyi_ec_supply `{renyiprivGS Σ} :=
  @rc_supply _ renyiprivGS_error.

Global Instance renyiprivGS_irisGS `{!renyiprivGS Σ} : renyiprivWpGS prob_lang Σ := {
  renyiprivWpGS_invGS := renyiprivGS_invG;
  state_interp σ := (heap_auth 1 σ.(heap) ∗ tapes_auth 1 σ.(tapes))%I;
  err_interp α ρ := (renyi_ec_supply (α, ρ))%I;
}.

(** Heap *)
Notation "l ↦{ dq } v" := (@ghost_map_elem _ _ _ _ _ renyiprivGS_heap renyiprivGS_heap_name l dq v)
...
(** Tapes *)
Notation "l ↪{ dq } v" := (@ghost_map_elem _ _ tape _ _ renyiprivGS_tapes renyiprivGS_tapes_name l dq v)
  (at level 20, format "l  ↪{ dq }  v") : bi_scope.
Notation "l ↪□ v" := (l ↪{ DfracDiscarded } v)%I
  (at level 20, format "l  ↪□  v") : bi_scope.
Notation "l ↪{# q } v" := (l ↪{ DfracOwn q } v)%I
...
(** User-level tapes *)
Definition nat_tape `{renyiprivGS Σ} l (N : nat) (ns : list nat) : iProp Σ :=
...
(*
Definition nat_spec_tape `{renyiprivGS Σ} l (N : nat) (ns : list nat) : iProp Σ :=
