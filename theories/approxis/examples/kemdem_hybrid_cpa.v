...
From clutch.approxis.examples Require Import
  valgroup diffie_hellman prf prf_cpa symmetric security_aux option xor
...
Definition enc_hyb : val :=
  λ: "pk" "msg",
    match: encaps "pk" with
      ...
      let: "k" := Snd "caps" in
      let: "c_dem" := (rf_enc Input SymOutput xor_struct) "k" "msg" in
      SOME ("c_dem", "c_kem")
    end.
...
Section logrel.
  ...
  Lemma rf_enc_sem_typed : ⊢
    REL (rf_enc SymOutput SymOutput xor_struct) <<
      (rf_enc SymOutput SymOutput xor_struct) :
        lrel_int → lrel_input → lrel_int * lrel_int.
  Proof with rel_pures_l; rel_pures_r.
    rewrite /rf_enc.
    rel_arrow_val.
    ...
    - rel_arrow_val. iIntros (v1 v2 [x' [eq1 eq2]]); subst...
      rel_apply random_function_sem_typed.
    - rel_values.
Qed.
...
Proof with rel_pures_l; rel_pures_r.
  ...
  subst; rel_pures_l; rel_pures_r.
  - rewrite /decaps... admit.
  - rewrite /enc... rel_apply refines_randU_l.
    iIntros (b Hbbound)... simpl_exp.
    simpl_exp. simpl_mult. rewrite /rf_enc...
    ...
    rewrite lookup_empty in Heqres. simpl in Heqres; subst...
    rel_apply refines_randU_l.
    iIntros (y Hybound)...
...
Proof with (rel_pures_l; rel_pures_r).
  ...
  rel_apply (refines_na_alloc (β ↪ₛN(N;[]) ∗ (cnt ↦ #0 ∗ cnt' ↦ₛ #0 ∨ cnt ↦ #1 ∗ cnt' ↦ₛ #1))%I
    (nroot.@"pk_real_real_tape")).
  iFrame. iSplitL; first (iLeft; iFrame).
  ...
  - rel_load_l; rel_load_r; rel_store_l; rel_store_r...
    ...
    rel_bind (rf_enc Input SymOutput xor_struct #k #msg).
    rel_apply refines_bind.
    2 : {
    ...
    repeat (rel_apply refines_app);
    first rel_apply rf_enc_sem_typed; rel_vals.
  - rel_apply refines_na_close. iFrame; iSplitL.
    {
...
Lemma pk_real_tape_DDH_real : ⊢
  REL pk_real_tape << Csenc_DDH_real : 
  lrel_G * (lrel_input → () + (() + (lrel_int * lrel_int) * (lrel_G * lrel_G))).
Proof with rel_pures_l; rel_pures_r. rewrite /pk_real_tape;
  ...
  rel_bind (rf_enc Input SymOutput xor_struct #k #msg).
  rel_apply refines_bind.
  { repeat (rel_apply refines_app); first (rel_apply rf_enc_sem_typed); rel_vals. }
...
Proof with rel_pures_l; rel_pures_r. rewrite /Csenc_DDH_rand.
  rewrite /Csenc. rewrite /eCsenc. simpl.
  rewrite /DDH_rand...
  rewrite /pk_rand_senc_delay.
  ...
  { rel_vals. }
  rel_apply refines_na_inv; iSplitL; first iAssumption.
  iIntros "[[[Hcnt [Hcnt' Hγ]]|[Hcnt [Hcnt' Hγ]]] Hclose]";
  ...
  rel_apply (refines_bind with "[-]").
  {
    ...
    repeat (rel_apply refines_app); first rel_apply rf_enc_sem_typed;
    rel_vals.
  }
  iIntros (v v') "Hrel"...
...
Definition pk_rand_senc_mult_free : expr :=
  ...
  if: #0 ≤ "msg" `and` "msg" ≤ #SymOutput then
    let: "cdem" := (rf_enc Input SymOutput xor_struct) "k" "msg" in
    SOME ("cdem", "ckem")
  else NONEV
  in ("pk", "query").
...
Proof with rel_pures_l; rel_pures_r.
  ...
  { rel_vals. }
  rel_apply refines_na_inv; iSplitL; first iAssumption.
  iIntros "[[Hγ [[Hcnt Hcnt']|[Hcnt Hcnt']]] Hclose]"...
  - rel_load_l; rel_load_r...
    rel_store_l; rel_store_r...
  ...
  destruct (log_g kg) as [k_msg ->].
    ...
    rewrite Hxbound'...
    rel_bind (rf_enc Input SymOutput xor_struct #k #x).
    rel_apply (refines_bind with "[-]").
    {
      rel_apply refines_na_close; iFrame.
      iSplitL; iFrame; first (iRight; iFrame).
...
Lemma adv_rand_syn_typed : ⊢ᵥ adv_rand :
  ((TMessage → TOption prf_cpa.TCipher) → τG * TOracle).
Proof.
  rewrite /adv_rand.
  apply Rec_val_typed.
...
Lemma pk_rand_senc_mult_free_adv_sym_cpa (adv : val) :
  ...
  ⊢ REL adv pk_rand_senc_mult_free <<
    CPA #true (λ: "oracle", adv (adv_rand "oracle")) (rf_scheme SymKey Input SymOutput xor_struct) #1 :
  lrel_bool.
Proof with rel_pures_l; rel_pures_r.
  iIntros "Hadvtyped".
  rewrite /adv_rand...
  ...
  rewrite /get_enc; rel_pures_r. rewrite /rf_enc; rel_pures_r.
  rewrite /prf_enc; rel_pures_r. rewrite /random_function; rel_pures_r.
  rel_apply refines_init_map_r.
  iIntros (mapref') "Hmap'".
  rewrite -/random_function; rel_pures_r.
  rewrite /q_calls; rel_pures_r.
  rewrite /get_card_message; rel_pures_r.
  ...
  set (P := (
      (cnt ↦ #0 ∗ cnt' ↦ₛ #0 ∗ cnt2 ↦ₛ #0 ∗ map_slist mapref' ∅)
    ∨ (cnt ↦ #1 ∗ cnt' ↦ₛ #1 ∗ cnt2 ↦ₛ #1 (*∗ ∃ M, map_slist mapref' M*))
  )%I).
  ...
  rel_apply refines_na_inv; iSplitL; first iAssumption.
  iIntros "[[[Hcnt [Hcnt' [Hcnt2 Hmap']]]|[Hcnt [Hcnt' Hcnt2]]] Hclose]".
  2 : {
...
Lemma rf_is_CPA_instantiated_adv_rand (adv : val)
  ...
  ⊢
  (REL (CPA #true (λ: "oracle", adv (adv_rand "oracle")) (rf_scheme SymKey Input SymOutput xor_struct) #1)
        <<
        (CPA #false (λ: "oracle", adv (adv_rand "oracle")) (rf_scheme SymKey Input SymOutput xor_struct) #1) : lrel_bool).
Proof. iStartProof.
...
Lemma rf_CPA_pk_rand_srand (adv : val) : 
  refines top adv
    adv ((lrel_G * (lrel_input → () + (() + lrel_int * lrel_int * (lrel_G * lrel_G)))) → lrel_bool)
  ⊢
  REL (CPA #false (λ: "oracle", adv (adv_rand "oracle")) (rf_scheme SymKey Input SymOutput xor_struct) #1)
  << adv pk_rand_srand : lrel_bool.
