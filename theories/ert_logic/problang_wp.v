...
From tachis.ert_logic Require Export expected_time_credits ert_weakestpre (* ectx_lifting *).
From tachis.prob_lang Require Export class_instances.
From tachis.prob_lang Require Export tactics lang notation.
From iris.prelude Require Import options.

Class ert_tachisGS Σ (cost : Costfun prob_lang) := HeapG {
...
Definition heap_auth `{ert_tachisGS Σ} :=
  @ghost_map_auth _ _ _ _ _ tachisGS_heap tachisGS_heap_name.
Definition tapes_auth `{ert_tachisGS Σ} :=
  @ghost_map_auth _ _ _ _ _ tachisGS_tapes tachisGS_tapes_name.

Global Instance tachisGS_irisGS `{!ert_tachisGS Σ F} : ertwpG prob_lang Σ := {
  ertwpG_invGS := tachisGS_invG;
  ertwpG_etcGS := ert_tachisGS_etc;
