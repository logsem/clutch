...
Section GenWp_mixin.
  Context {S : Sig}.
  Canonical Structure gen_ectxi_lang_gwp := gen_ectxi_lang S.
  Canonical Structure gen_ectx_lang_gwp := gen_ectx_lang S.
  Canonical Structure gen_lang_gwp := gen_lang S.
...
Structure GenWp (S : Sig) Σ `{invGS_gen hlc Σ} := {
  gwp_laters : bool;

  gwp : coPset → expr  → (val → iProp Σ) → iProp Σ;
  gwp_pointsto : loc → dfrac → val → iProp Σ;

  gwp_mixin : GenWpMixin (S := S) gwp_laters gwp gwp_pointsto;
}.

Arguments Build_GenWp {_ _ _ _ _ _ _}.
Arguments gwp {_ _ _ _}.
Arguments gwp_pointsto {_ _ _ _}.
Arguments gwp_laters {_ _ _ _}.
