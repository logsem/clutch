...
From clutch.gen_prob_lang Require Import lang tactics notation class_instances.
Set Default Proof Using "Type*".

(* The generic language [gen_lang S] is parametric in a distribution signature
   [S : Sig].  As in [class_instances.v], the [GwpTactics*] typeclasses below are
   declared inside a [Section] over [S : Sig] with the
   [gen_ectxi_lang/gen_ectx_lang/gen_lang] canonical-structure chain in scope.
   Class fields that mention a *language* ([LanguageCtx], [PureExec], [fill])
   are explicitly annotated [(Λ := gen_lang S)] so the classes are parametric in
   [S]; downstream the WP layer fixes a concrete signature [Sg] via [diffprivGS].
   The [Tactic Notation]s/[Ltac]s remain at top level — they are purely
   syntactic — and feed the extra leading [S] argument of the [tac_wp_*] lemmas a
   placeholder [_], unified from the goal. *)

Section classes.
  Context (S : Sig).
  Canonical Structure gen_ectxi_lang_S_cls := gen_ectxi_lang S.
  Canonical Structure gen_ectx_lang_S_cls := gen_ectx_lang S.
  Canonical Structure gen_lang_S_cls := gen_lang S.
  ...
  Class GwpTacticsBind (Σ : gFunctors) (A : Type) `{!invGS_gen hlc Σ} (gwp : A → coPset → expr → (val → iProp Σ) → iProp Σ)  := {
      wptac_wp_bind K `{!LanguageCtx (Λ := gen_lang S) K} E e Φ a :
        gwp a E e (λ v, gwp a E (K (of_val v)) Φ ) ⊢ gwp a E (K e) Φ
  }.

  Class GwpTacticsPure (Σ : gFunctors) A (laters : bool) (gwp : A → coPset → expr → (val → iProp Σ) → iProp Σ) := {
    wptac_wp_pure_step E e1 e2 φ n Φ a :
      PureExec (Λ := gen_lang S) φ n e1 e2 →
      φ →
      ▷^(if laters then n else 0) (gwp a E e2 Φ) ⊢ (gwp a E e1 Φ);
  }.

  (** Heap *)
  Class GwpTacticsHeap (Σ : gFunctors) A (laters : bool) (gwp : A → coPset → expr → (val → iProp Σ) → iProp Σ):= {
    ...
    wptac_wp_load E v l dq a Φ :
      ...
      gwp a E (Deref (Val $ LitV $ LitLoc l)) Φ;
  ...
  (** Sample tapes.  Generic over the family index [i : nat], parameter value
      [pv : val] and presampled outcomes [list val].  The tape predicate
      [wptac_mapsto_tape] is a *parameter* of the class (mirroring how
      [prob_lang]'s [GwpTacticsTapes] abstracts the tape points-to), so no ghost
      map is hard-coded here. *)
  Class GwpTacticsSampleTape (Σ : gFunctors) A (laters : bool) (gwp : A → coPset → expr → (val → iProp Σ) → iProp Σ):= {
    wptac_mapsto_tape : loc → dfrac → nat → val → (list val) → iProp Σ;

    wptac_wp_alloc_sample_tape E i (pv : val) a Φ :
      True -∗
      (▷?laters (∀ l, (wptac_mapsto_tape l (DfracOwn 1) i pv nil) -∗ Φ (LitV (LitLbl l))%V)) -∗
      gwp a E (AllocSampleTape i (Val pv)) Φ;

    wptac_wp_sample_tape E i pv (x : val) xs l dq a Φ :
      (▷ wptac_mapsto_tape l dq i pv (x :: xs)) -∗
      (▷?laters ((wptac_mapsto_tape l dq i pv xs) -∗ Φ x)) -∗
      gwp a E (Sample i (Val pv) (Val (LitV (LitLbl l)))) Φ;
  }.
...
Section wp_tactics.
  Context (S : Sig).
  Canonical Structure gen_ectxi_lang_S_wpt := gen_ectxi_lang S.
  Canonical Structure gen_ectx_lang_S_wpt := gen_ectx_lang S.
  Canonical Structure gen_lang_S_wpt := gen_lang S.
  ...
  Lemma tac_wp_pure_later laters `{!GwpTacticsPure S Σ A laters gwp} Δ Δ' E
    (K : list (ectxi_language.ectx_item (gen_ectxi_lang S))) e1 e2 φ n Φ a :
    PureExec (Λ := gen_lang S) φ n e1 e2 →
    φ →
...
Section wp_bind_tactics.
  Context (S : Sig).
  Canonical Structure gen_ectxi_lang_S_bnd := gen_ectxi_lang S.
  Canonical Structure gen_ectx_lang_S_bnd := gen_ectx_lang S.
  Canonical Structure gen_lang_S_bnd := gen_lang S.
  Context `{GwpTacticsBind S Σ A hlc gwp}.
  ...
  Lemma tac_wp_bind (K : list (ectxi_language.ectx_item (gen_ectxi_lang S))) Δ E Φ e f a :
...
Ltac wp_finish :=
  ...
  rewrite ?[wptac_mapsto_tape _ _ _ _ _]/=;
...
Section heap_tactics.
  Context (S : Sig).
  Canonical Structure gen_ectxi_lang_S_heap := gen_ectxi_lang S.
  Canonical Structure gen_ectx_lang_S_heap := gen_ectx_lang S.
  Canonical Structure gen_lang_S_heap := gen_lang S.
  Context `{GwpTacticsBase Σ A hlc gwp, GwpTacticsBind S Σ A hlc gwp, !GwpTacticsHeap Σ A laters gwp}.
  ...
  Lemma tac_wp_alloc Δ Δ' E j (K : list (ectxi_language.ectx_item (gen_ectxi_lang S))) v Φ a :
  ...
  Lemma tac_wp_allocN Δ Δ' E j (K : list (ectxi_language.ectx_item (gen_ectxi_lang S))) v n Φ a :
    (0 < n)%Z →
    MaybeIntoLaterNEnvs (if laters then 1 else 0) Δ Δ' →
    (∀ l,
  ...
  Lemma tac_wp_load Δ Δ' E i (K : list (ectxi_language.ectx_item (gen_ectxi_lang S))) b l dq v Φ a :
    MaybeIntoLaterNEnvs (if laters then 1 else 0) Δ Δ' →
    envs_lookup i Δ' = Some (b, wptac_mapsto l dq v) →
    envs_entails Δ' (WP fill K (Val v) @ a; E {{ Φ }}) →
    envs_entails Δ (WP fill K (Deref (Val $ LitV $ LitLoc l)) @ a; E {{ Φ }}).
  Proof.
  ...
  Lemma tac_wp_store Δ Δ' E i (K : list (ectxi_language.ectx_item (gen_ectxi_lang S))) l v v' Φ a :
    MaybeIntoLaterNEnvs (if laters then 1 else 0) Δ Δ' →
...
Tactic Notation "wp_load" :=
  ...
  | |- envs_entails _ (wp ?s ?E ?e ?Q) =>
    first
      ...
      |fail 1 "wp_load: cannot find 'Deref' in" e];
    [tc_solve
    |solve_wptac_mapsto ()
    |wp_finish]
  | |- envs_entails _ (twp ?s ?E ?e ?Q) =>
...
Section sample_tape_tactics.
  Context (S : Sig).
  Canonical Structure gen_ectxi_lang_S_stp := gen_ectxi_lang S.
  Canonical Structure gen_ectx_lang_S_stp := gen_ectx_lang S.
  Canonical Structure gen_lang_S_stp := gen_lang S.
  Context `{GwpTacticsBase Σ A hlc gwp, GwpTacticsBind S Σ A hlc gwp, !GwpTacticsSampleTape Σ A laters gwp}.
  ...
  Lemma tac_wp_alloc_sample_tape Δ Δ' E j (K : list (ectxi_language.ectx_item (gen_ectxi_lang S))) i (pv : val) Φ a :
    MaybeIntoLaterNEnvs (if laters then 1 else 0) Δ Δ' →
    (∀ l,
        match envs_app false (Esnoc Enil j (wptac_mapsto_tape l (DfracOwn 1) i pv nil)) Δ' with
        | Some Δ'' =>
            envs_entails Δ'' (WP fill K (Val $ LitV $ LitLbl l) @ a ; E {{ Φ }})
        | None => False
        end) →
    envs_entails Δ (WP fill K (AllocSampleTape i (Val pv)) @ a; E {{ Φ }}).
  Proof.
    rewrite envs_entails_unseal=> ? HΔ.
    rewrite -wptac_wp_bind.
    eapply bi.wand_apply.
    { by apply bi.wand_entails, wptac_wp_alloc_sample_tape. }
    rewrite left_id into_laterN_env_sound.
    apply bi.laterN_mono, bi.forall_intro=> l.
    specialize (HΔ l).
    destruct (envs_app _ _ _) as [Δ''|] eqn:HΔ'; [| contradiction].
  ...
  Lemma tac_wp_sample_tape Δ1 Δ2 E i (K : list (ectxi_language.ectx_item (gen_ectxi_lang S))) l idx (pv x : val) xs Φ a :
    MaybeIntoLaterNEnvs (if laters then 1 else 0) Δ1 Δ2 →
    envs_lookup i Δ2 = Some (false, wptac_mapsto_tape l (DfracOwn 1) idx pv (x::xs)) ->
    (match envs_simple_replace i false (Esnoc Enil i (wptac_mapsto_tape l (DfracOwn 1) idx pv xs)) Δ2 with
    | Some Δ3 =>
        envs_entails Δ3 (WP fill K (Val x) @ a; E {{ Φ }})
    | None    => False
    end) →
    envs_entails Δ1 (gwp a E (fill K (Sample idx (Val pv) (Val (LitV (LitLbl l))))) Φ ).
  Proof.
    rewrite envs_entails_unseal=> ? Hi HΔ.
    destruct (envs_simple_replace _ _ _ _) as [Δ3|] eqn:HΔ3; last done.
    rewrite -wptac_wp_bind.
    eapply bi.wand_apply.
    { by apply bi.wand_entails, wptac_wp_sample_tape. }
    rewrite into_laterN_env_sound.
    destruct laters.
    - rewrite -bi.later_sep.
      apply bi.later_mono.
...
End sample_tape_tactics.


Tactic Notation "wp_alloc_sample_tape" ident(l) "as" constr(H) :=
  let Htmp := iFresh in
  let finish _ :=
    first [intros l | fail 1 "wp_alloc_sample_tape:" l "not fresh"];
    pm_reduce;
    lazymatch goal with
    | |- False => fail 1 "wp_alloc_sample_tape:" H "not fresh"
    | _ => iDestructHyp Htmp as H; wp_finish
    end in
  wp_pures;
lazymatch goal with
  | |- envs_entails _ (wp ?s ?E ?e ?Q) =>
        first
          [reshape_expr e ltac:(fun K e' => eapply (tac_wp_alloc_sample_tape _ _ _ _ Htmp K))
          |fail 1 "wp_alloc_sample_tape: cannot find 'AllocSampleTape' in" e];
        [tc_solve
        |finish ()]
| |- envs_entails _ (twp ?s ?E ?e ?Q) =>
        first
        ...
        [tc_solve
        |finish ()]
  | _ => fail "wp_alloc_sample_tape: not a 'wp'"
  end.


Tactic Notation "wp_alloc_sample_tape" ident(l) :=
  wp_alloc_sample_tape l as "?".

Tactic Notation "wp_sample_tape" "as" constr(H) :=
  let Htmp := iFresh in
  let solve_wptac_mapsto_tape _ :=
    let l := match goal with |- _ = Some (_, (wptac_mapsto_tape ?l _ _ _ (_ :: _))%I) => l end in
    iAssumptionCore || fail "wp_sample_tape: cannot find" l "↪ ?" in
  let finish _ :=
    pm_reduce;
    lazymatch goal with
    | |- False => fail 1 "wp_sample_tape:" H "not fresh"
    | _ => iDestructHyp Htmp as H; wp_finish
    end in
  wp_pures;
  lazymatch goal with
  | |- envs_entails _ (wp ?s ?E ?e ?Q) =>
      first
        [reshape_expr e ltac:(fun K e' => eapply (tac_wp_sample_tape _ _ _ _ K))
        |fail 1 "wp_sample_tape: cannot find 'Sample' in" e];
      [tc_solve
  ...
  | |- envs_entails _ (twp ?s ?E ?e ?Q) =>
      ...
      | solve_wptac_mapsto_tape ()
      | finish ()]
  | _ => fail "wp_sample_tape: not a 'wp'"
  end.

Tactic Notation "wp_sample_tape" :=
  wp_sample_tape as "?".
