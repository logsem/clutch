(** Resources required to track a [gen_prob_lang] spec configuration.
    Generalized from [prob_lang]: the per-distribution tape ghost maps
    (tapes / tapes_laplace / …) collapse into a single generic [stape] map. *)
...
Class specG_prob_lang Σ := SpecGS {
  ...
  #[local] specG_prob_lang_tapes :: ghost_mapG Σ loc stape;
...
Class specGpreS Σ := SpecGPreS {
  ...
  specGpreS_tapes :: ghost_mapG Σ loc stape;
}.
...
(** Spec tapes (one generic tape map) *)
...
Section theory.
  ...
  Lemma spec_auth_tape_alloc e σ (t : stape) :
    spec_auth (e, σ) ==∗
    spec_auth (e, state_upd_stapes <[fresh_loc σ.(stapes) := t]> σ) ∗ fresh_loc σ.(stapes) ↪ₛ t.
  Proof.
    iIntros "(? & ? & Htapes) /=".
    iMod (ghost_map_insert (fresh_loc σ.(stapes)) with "Htapes") as "[H Hl]".
    { apply not_elem_of_dom, fresh_loc_is_fresh. }
    by iFrame.
  Qed.
...
Lemma spec_ra_init e σ `{specGpreS Σ} :
  ⊢ |==> ∃ _ : specG_prob_lang Σ,
      spec_auth (e, σ) ∗ ⤇ e ∗
      ([∗ map] l ↦ v ∈ σ.(heap), l ↦ₛ v) ∗
      ([∗ map] α ↦ t ∈ σ.(stapes), α ↪ₛ t).
Proof.
  iMod (own_alloc ((● (Excl' e)) ⋅ (◯ (Excl' e)))) as "(%γp & Hprog_auth & Hprog_frag)".
  { by apply auth_both_valid_discrete. }
  iMod (ghost_map_alloc σ.(heap)) as "[%γH [Hh Hls]]".
  iMod (ghost_map_alloc σ.(stapes)) as "[%γT [Ht Hαs]]".
  iExists (SpecGS _ _ γp _ _ γH γT).
  by iFrame.
Qed.

#[global] Instance spec_rules_spec_updateGS `{!specG_prob_lang Σ} (S : Sig) :
  spec_updateGS (lang_markov (gen_lang S)) Σ.
Proof. constructor. exact spec_auth. Defined.
