...
(*   double_summable a → double_summable (Rabs ∘ a). *)
(* Proof. *)
...
(*       { right. Admitted. (* apply eq_bigr. intros; rewrite sum_n_bigop. done. } *) *)
...
(*   { eapply ex_series_row. apply double_summable_by_flip => //=. } *)
(*   intros n => //=. *)
(* Qed. *)
...
(*       ** exists N => l m. inversion 1; subst. *)
(*          *** inversion 1; subst. right. done. *)
(*          *** eapply HN; auto. *)
(*       ** edestruct (COV _ Hneq) as (N'&?). *)
(*          exists (max N N') => l m. inversion 1; subst; inversion 1; subst. *)
(*          *** left. exists N'. split; auto. apply Max.le_max_r. *)
...
(* Variable (INJ: ∀ n n', a (σ n) <> 0 → σ n = σ n' → n = n'). *)
(* Variable (COV: ∀ n, a n <> 0 → ∃ m, σ m = n). *)
(* Variable (EXS: ex_series (Rabs ∘ a ∘ σ)). *)
...
(* (*   { eapply Rle_abs. } *) *)
(* (*   etransitivity; eauto. *) *)
(* (*   eapply is_series_partial_pos in His. *) *)
(* (*   *  eapply Rle_ge.  Rabs_pos. *) *)
(* (*   * eapply is_series_ext; last eassumption. *) *)
(* (*     intros ? => //=. by rewrite Rabs_Rabsolu. *) *)
(* (* Qed. *) *)
...
(*   assert (ex_series (a ∘ σ)) as (v'&Hconv). *)
...
(*     edestruct (Cauchy_ex_series (Rabs ∘ a ∘ σ)) as (N0&IHN). *)
(*     { exists v; eauto. } *)
...
(*       eapply IHN; auto. lia. *)
(*     - eapply HN2. *)
(*       rewrite /N. etransitivity; first apply Max.le_max_r. lia. *)
(*   } *)
...
(*       transitivity (Lim_seq *)
(*                       (λ k, norm (sum_n (λ j, sum_n (λ k, a (j, k)) k) l - sum_n (a ∘ σ) N))); *)
(*         last first. *)
(*       { *)
...
Lemma series_double_swap a (DS: double_summable a):
  is_series (λ j, Series (λ k, a (j, k))) (Series (λ k, Series (λ j, a (j, k)))).
Proof. Admitted.
  (* by apply double_swap.series_double_swap. Qed. *)
