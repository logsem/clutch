...
Record state : Type := {
  heap  : gmap loc val;
  (* The nat at each location is the default tape bound for this namespace.
     We don't actually even care that each tape in the namespace has this bound, so
     there's no need to remove it from tape itself. *)
  tapes : gmap loc (nat * gmap Z tape);
}.
...
Definition state_upd_tapes (f : gmap loc (nat * gmap Z tape) → gmap loc (nat * gmap Z tape)) (σ : state) : state :=
...
Definition unif_expr (N : nat) (σ : state) : distr (expr * state) :=
  dmap (λ n : fin _, (Val $ LitV $ LitInt n, σ)) (dunifP N).

(* NB: Initializing a single tape should be provable as an erasable step
    ie. going from 𝜄 ↦ (M, f) to 𝜄 ↦ (M, <[𝛼 := (M; [])]> f) provided that f 𝛼 = None.

    Hence looking up from an empty tape and looking up from an uninitialized tape
    w/ the same bound should be the same. *)
...
Definition head_step (e1 : expr) (σ1 : state) : distr (expr * state) :=
  ...
  | Rand (Val (LitV (LitInt N))) (Val (LitV LitUnit)) => unif_expr (Z.to_nat N) σ1
  | AllocTape (Val (LitV (LitInt z))) =>
      let ι := fresh_loc σ1.(tapes) in
      dret (Val $ LitV $ LitLbl ι, state_upd_tapes <[ι := (Z.to_nat z, ∅)]> σ1)
  (* Labelled sampling, no namespaces, conditional on tape contents *)
  | Rand (Val (LitV (LitInt N))) (Val (LitV (LitLbl l))) =>
      match (σ1.(tapes) !! l : option (nat * gmap Z tape))with
      (* l is allocated. *)
      | Some (MDef, Tapes) =>
          match ((Tapes !! (0 : Z)) : option tape) with
          (* l is allocated; l !! 0 is initialized. *)
          | Some (M; ns) =>
             if bool_decide (M = Z.to_nat N) then
               (* l is allocated; l !! 0 is initialized; l !! 0 bound matches. *)
               match ns with
                (* l is allocated; l !! 0 is initialized; l !! 0 bound matches; tape is nonempty. *)
               | n :: ns =>
                   dret (Val $ LitV $ LitInt $ fin_to_nat n,
                         state_upd_tapes <[l:=(MDef, <[(0 : Z) := (M; ns)]> Tapes)]> σ1)
                (* l is allocated; l !! 0 is initialized; l !! 0 bound matches; tape is empty. *)
               | [] => unif_expr M σ1
               end
             (* l is allocated; l !! 0 is initialized; l !! 0 bound does not match. *)
             else unif_expr (Z.to_nat N) σ1
          (* l is allocated; l !! 0 is no initialized. *)
          | None => unif_expr (Z.to_nat N) σ1
          end
      (* l is no allocated. *)
      | None => dzero
      end

  (* Labelled sampling, with namespaces, conditional on tape contents.
     This is the same as the prior case, however it uses the ι entry in the namespace
     rather than the 0 entry. *)
  | Rand (Val (LitV (LitInt N))) (Val (PairV (LitV (LitLbl l)) (LitV (LitInt ι)))) =>
      match (σ1.(tapes) !! l : option (nat * gmap Z tape))with
      | Some (MDef, Tapes) =>
          match ((Tapes !! ι) : option tape) with
          | Some (M; ns) =>
             if bool_decide (M = Z.to_nat N) then
               match ns with
               | n :: ns =>
                   dret (Val $ LitV $ LitInt $ fin_to_nat n,
                         state_upd_tapes <[l:=(MDef, <[ι := (M; ns)]> Tapes)]>  σ1)
               | [] => unif_expr M σ1
               end
             else unif_expr (Z.to_nat N) σ1
          | None => unif_expr (Z.to_nat N) σ1
