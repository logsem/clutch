...
2. Install a new project-local switch.
```
...
The development relies on axioms for classical reasoning and an axiomatization of the reals numbers, both found in Rocq's standard library. For example, the following list is produced when executing the command `Print Assumptions adv_DHKE_real.` in [`theories/prob_eff_lang/probblaze/examples/DH_KE/advantage_dhke_lazy.v`](theories/prob_eff_lang/probblaze/examples/DH_KE/advantage_dhke_lazy.v):
